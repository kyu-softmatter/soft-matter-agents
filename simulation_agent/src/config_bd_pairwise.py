...
def operating_point() -> dict:
    ...
    computed = [
        ...
        {"name": "total_simulated_time_point", "value": 10000, "unit": "s", "source": "computed:cap_times_brownian_time", "formula": "run_cap_factor*brownian_time", "inputs": ["run_cap_factor", "brownian_time"],
         "note": "ten Brownian times: the planned record length, and the cap. The run goes to it; the relaxation time is read from the saved curve afterwards, and a crossing that does not happen before the plateau window is reported not converged"},
        ...
        {"name": "plateau_window_start", "value": 9000, "unit": "s", "source": "computed:record_minus_window", "formula": "total_simulated_time_point - relaxation_fit_window", "inputs": ["total_simulated_time_point", "relaxation_fit_window"],
         "note": "the psi6 crossing must fall before this time for the plateau to be a plateau; the success criterion compares against it"},
        ...
        {"name": "box_length_point", "value": 300, "unit": "um", "source": "computed:rows_times_lattice_constant", "formula": "rows_x*lattice_constant", "inputs": ["rows_x", "lattice_constant"],
         "note": "the box's x edge, which the trajectory writer records; the y edge is rows_y * sqrt(3)/2 * lattice_constant and is derived by the backend, irrational and unwritable at one figure"},
        ...
        {"name": "steps_per_run", "value": 2e6, "unit": "1", "source": "computed:cap_over_step", "formula": "total_simulated_time_point/dt_max_curvature", "inputs": ["total_simulated_time_point", "dt_max_curvature"],
        ...
        {"name": "frames_per_run", "value": 100, "unit": "1", "source": "computed:cap_over_save_interval", "formula": "total_simulated_time_point/save_interval_max", "inputs": ["total_simulated_time_point", "save_interval_max"]},
    ...
    point = [("integration_timestep", "dt_max_curvature"), ("total_simulated_time", "total_simulated_time_point"), ("save_interval", "save_interval_max"),
             ("box_length", "box_length_point"), ("mean_spacing", "mean_spacing"), ("plateau_window_start", "plateau_window_start"),
...
def build_plan(qid: str, created_at: str, revision: int = 1) -> dict:
    ...
    extra = ["temperature", "viscosity", "bead_diameter", "diffusivity", "brownian_time", "levels_per_decade",
    ...
    numbers.append(cards.num("smoke_record_fraction", 0.01, "1", "computed:decision_smoke_fraction",
                             formula="0.01*levels_per_decade", inputs=[("levels_per_decade", g["levels_per_decade"])], precision="order_of_magnitude",
                             note="the smoke run is one hundredth of the record at the same save interval: 100 s, 2e4 steps, one saved frame. S5's decision, written as arithmetic on a carried number so the validator can recompute it; it exists to time the integrator and to walk the pipeline, and its output never becomes a result card"))
    ...
    card = cards.head(
        ...
        actions=[
            ...
            {"id": "integrate", "device": "hoomd_backend",
             "action": "integrate overdamped Brownian dynamics at integration_timestep for total_simulated_time, saving UNWRAPPED positions every save_interval (the trajectory convention; psi6 folds them into the box at read time)",
             "reversible": True, "parameters": ["integration_timestep", "save_interval", "total_simulated_time", "temperature", "viscosity", "bead_diameter"], "tier": 1},
            {"id": "estimate_relaxation_time", "device": "hoomd_backend",
             "action": "per frame fold positions into the box and compute psi6 per particle from its Voronoi neighbours with periodic images, average |psi6_j| over particles; take the plateau as the mean over the last relaxation_fit_window of the record; report the first time psi6(t) reaches plateau_fraction of it, per seed, and the mean and standard error over seeds per cell",
        ...
        stop_criteria=[
            {"id": "planned_duration_reached", "metric": "simulated_time", "comparator": ">=", "number": "total_simulated_time_point", "on_met": "complete",
             "statement": "the run ends at the planned record length, ten Brownian times; the relaxation time is read from the saved psi6 curve afterwards, because the plateau it is read against is not knowable during the run"},
        ...
        success_criteria=[
            {"id": "converged_before_window", "metric": "time_of_plateau_crossing", "comparator": "<", "number": "plateau_window_start",
             "statement": "in every seed of a cell psi6 reached the declared fraction of its plateau before the plateau window began, so the plateau was a plateau; a cell where it did not is reported not converged and its time is a lower bound, not a value"},
            {"id": "statistics_met", "metric": "relative_standard_error_of_relaxation_time", "comparator": "<=", "number": "target_rel_error",

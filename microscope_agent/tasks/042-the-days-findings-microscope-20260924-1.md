# 042 — the day's findings for the store: microscope-20260924-1

Written by `manager-microscope-20260924-1`. You read this; you do not edit it
(§6.2-2).

**Assigned to `microscope-20260924-1`.** The person asked, as the day's last request, that
...
- `Kinetix_red`'s serial, `A24M723015`, and chip name, read in run
  `-002` at step 4.6 (a self-report)
- the dark test on this load: Aura master off with a line at maximum
  against the line off (step 4.3)
- the Aura line names and the intensity upper limit of 1000, with `100`
  read back as `100` (step 4.4)
- the frame period, from the camera's timestamps and `ElapsedTime-ms`
- **bit depth UNMEASURED**: the dia-lamp frame did not saturate, and
  `bits: 10` in the log is a script flaw, not a reading
- the nosepiece and Lapp mirror read-backs at load
- run `-001`'s core-lifetime failure, as a document fact about the driver
- the five-minute run's readings, if it has run: its self-reports, with
  its photostability curve as an `observable_value`, not for the store

## What to do

**List everything you established today** in one file,
**`microscope_agent/findings/microscope-20260924-1-20260924.json`**, in the shape

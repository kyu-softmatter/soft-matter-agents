...
| ~~**Bead diameter**~~ | **SETTLED 2026-09-19, and not by the person answering.** `tracer_diameter_measured` is `calibration:` **E2** — 5 µm measured on the bottle in use, with a **CV upper bound of 0.02** as a second number. The 5 µm / 2 µm split is closed and the simulation side already cites the store; **this side still cites `operator_recall:` E5 and is the one that is behind.** The re-pin moves it. Keep the CV: nothing here has a spread for this number and A2's statistics is where a spread earns its keep. `tracer_number_density` still waits on the dilution, below |
| ~~**Bead lot number**~~ | **CLOSED as a debt on 2026-09-19, and closed NEGATIVE.** `bottle_label_states_no_product`, E3, `operator_read` — the person examined the bottle and **the label states no product identity.** That is a reading and not a silence, which is why it is an entry: it converts the identification from *unconfirmed-and-pending* into **unconfirmed-and-permanent**. **`spec:<lot>` is not coming**, and anything still written as though it were describes a future that was cancelled. The diameter did not need it — an individual measurement lands **above** a catalogue identity, because it carries no same-stock assumption |
...
4. **regenerate the Markdown**, because the generator wrote `DRAFT` into it
   and the promotion does not reach back

**Step 4 is not bookkeeping.** `to_card()` writes both files and the status
is in both, so a hand-raised JSON leaves the Markdown saying `DRAFT` — and
P3 says the JSON wins, which makes the Markdown wrong rather than stale. The
first plan card hit this on 2026-09-21 and the seat regenerated by calling
`to_markdown` directly, **which is a step that runs on whoever remembers
it.** That is the drift P3 exists to make harmless, reappearing as a
different shape.

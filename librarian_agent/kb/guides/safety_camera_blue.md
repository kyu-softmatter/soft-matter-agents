# Safety guide: Camera on the blue arm (Photometrics Kinetix)
...
- **The cameras' manual.** The archive's Camera folder holds only installers -- PVCam 3.10.1.1 (SDK), PVCam 3.10.2.7 and an evaluation tool named PmPrimeCamEval -- while the store holds a Kinetix 22 datasheet. The tool's name suggests a different camera line and does not establish one; that is recorded and not resolved by guessing. What would close it: the Photometrics Kinetix user manual.

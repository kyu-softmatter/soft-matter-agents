# Safety guide: Microscope stand (Nikon Ti2-E)
...
- Ti2 Control must be installed before the microscope is connected to the computer by USB, or the driver may not install and the microscope may not be recognised; and the USB cable to the microscope must not be unplugged once Ti2 Control is running.  
  *Source: Nikon Ti2 Control Ver2.90 manual (English), pages 2, 3, 6 (PDF 10, 11, 14).*
- On the Ti2-E the light path has four addresses: 1 is the eyepiece observation port; 2 the right side port; 3, with the optical-path split prism, sends 80 per cent to the left side port and 20 per cent to the eyepieces, or is the bottom port on the Ti2-E/B; and 4 is the left side port. So two addresses can put light into the eyepieces. The position the path takes at start-up is a Ti2 Control setting.  
  *Source: Nikon Ti2 Control Ver2.90 manual (English), pages 67, 75 (PDF 75, 83).*
- In Ti2 Control, each the nosepiece address can be set to move the objective to its escape position before that address passes when the the nosepiece rotates, and the escape distance used when changing the specimen is also a setting there.  
  *Source: Nikon Ti2 Control Ver2.90 manual (English), page 73 (PDF 81).*
- In Ti2 Control, whether the fluorescence turret's shutter is open or closed when the microscope starts is a setting.  
  *Source: Nikon Ti2 Control Ver2.90 manual (English), page 75 (PDF 83).*
...
- **The Ti2-E microscope stand's hardware manual.** The stand carries the motorised focus and the nosepiece, whose collision with a coverslip is the bench's worst irreversible risk, and the only Nikon document in the archive is the control software's manual. What would close it: the Nikon Ti2-E instruction manual.
- The Ti2 Control manual's cautions are about installing the software and the USB cable, not about hazards. A Japanese copy of the same manual is in the archive and was not read, being a translation.

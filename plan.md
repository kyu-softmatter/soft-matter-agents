...
|---|---|---|
| 82 | whether **every third-party module an agent imports is declared in `pyproject.toml`**, across the `[project]` list and the pixi tables. **Asymmetric, and the asymmetry is the design**: an import with no declaration **FAILS**, because the code cannot run in that environment on another machine; a declaration with no import **reports**, because a person may provision a session ahead of the work, which is what `scipy` is today. **AST and not regex** -- a first pass returned five phantoms (`an`, `the`, `it`, `one`, `fields`) by reading *from ...* in docstring prose as an import, and counted sibling modules as third-party; with AST and `-`/`_` normalisation it reports 8 declarations, 7 imported modules, 53 files, and `gsd` as the single undeclared import | manager-simulation |

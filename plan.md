...
구현 순서는 **사서 → 현미경 → 시뮬레이션 → 브리지**다. 2026-09-17에 바꿨다 — 원래는 현미경이 먼저였다.

**마일스톤 이름은 순서가 아니다.** M0–M5는 일의 덩어리에 붙은 이름이고, 실행 순서는 아래 표의 첫 열이다. 순서를 이름에 담으면 순서를 바꿀 때마다 이 문서와 검증기 메시지와 네 개의 `CLAUDE.md`에 흩어진 "M3가 만드는 것" 같은 참조가 전부 조용히 다른 것을 가리키게 된다. 이름은 고정하고 순서만 바꾼다.

| 순서 | | 내용 | 완료 조건 |
|---|---|---|---|
| ✔ | **M0** | **계약 먼저.** 카드 스키마 + `units.json`/`units.md` + `validate.py`(§8의 검사 전부) + `capabilities/` 골격 | **완료.** 사람이 손으로 쓴 goal → axis → synthesis → plan → 승인 → result 왕복이 검증기를 통과하고(`0 failed`), `rejected/`의 카드가 **한 장도 빠짐없이** 거절된다(`--expect-fail`). 에이전트 코드는 없다. — 개수를 적지 않는 이유: 2026-09-17에 손으로 센 숫자가 세 곳에서 어긋났다. 셀 수 있는 것은 검증기에서 읽는다. |
| **1** | **M3** | **사서 에이전트.** entry `validity` 기계 판독화 → `kb/staging/` 분해 → 읽기 전용 MCP 서버(도구 4개, `caller_id` 격리) → `gaps` 산출 → 충돌 탐지 → 외부 검색·증류 → `kb/exports/` 발행 | 실제 질의 1건에 entries + gaps가 돌아오고, 같은 `(질의, kb_version)`을 두 번 불러 **바이트 단위로 같은** 답이 온다. E6이 KB에 없다. §12가 비어 있다. |
| **2** | **M1** | **현미경 에이전트.** S2 정교화 → S3.0 스크리닝 → 축별 해석 → S4 종합 → S5 산출 → 승인 → 실행. `envelope/safety.json` 기계 판독화(**사람이 쓴다**, §10.3 규칙 4), 광경로 표와 `capabilities/`의 연결(검사 38), 승인 게이트 hook | 승인 없이 Tier 2 호출이 **실제로 차단**된다. mock 백엔드로 파이프라인 1회, 그다음 실측 1건이 편차 기록과 함께 남는다. **그리고 사서를 끈 상태로 1회** — 카드가 `degraded: ["librarian_agent"]`를 달고 검증기를 통과한다. |
| **3** | **M2** | **시뮬레이션 에이전트.** 같은 5단 파이프라인, 축 A1–A5 + A7(구동 구성에서만 적용되고, 평형 구성에서는 기권한다. A6 없음 — §4.5.3), HOOMD 백엔드 | 목표 1건 → 검증 통과한 plan → 스모크런 → 수렴 증거. **사서를 끈 상태로 1회**는 M1과 같이 요구된다. |
| **4** | **M4** | **브리지 왕복 1라운드.** 해시, 단위·무차원수 일관성, 대응 가능성 | 한쪽 result → 반대쪽 plan → 결과가 같은 관측량으로 비교된다(S4). |
| **5** | **M5** | **회고 루프.** `failures.jsonl` → 사서 증류 → `kb/lessons/`, 편향 지표 4종(§8.2) | 같은 질문의 두 번째 왕복이 더 빠르고 모순이 없다(S5). lesson이 후보를 지우지 않음이 검사로 확인된다(검사 31). |

**왜 바꿨나.** 사서는 **협업 의존이 0인 유일한 에이전트다**(§4.3.2) — mock 상대도 스텁도 없이 완결되므로 언제 만들어도 된다. 그리고 지금 막혀 있는 것이 사서 쪽에 있다: `kb/staging/`의 평평한 표 두 개가 "질의하는 쪽이 정해지면 원자적 entry로 분해한다"는 상태로 멈춰 있고(§11.1), 그 질의하는 쪽이 사서다. 현미경의 S3.0이 읽어야 하는 것이 바로 그 표이므로, **사서를 먼저 세우는 것은 현미경을 미루는 것이 아니라 푸는 것이다.**

**대신 잃는 것 둘.** 원래 순서가 공짜로 주던 것이 없어진다. 대체할 수 있는 것은 대체하고, 못 하는 것은 잃는 것으로 적어 둔다.

1. **축소 모드가 더 이상 기본값이 아니다.** 원래 순서에서 현미경과 시뮬레이션은 사서가 없는 세계에서 태어나므로 `degraded: ["librarian_agent"]`가 유일한 경로였다. 이제는 사서가 살아 있는 상태에서 태어나고, 축소 모드는 아무도 지나가지 않는 분기가 된다. **나중에 덧붙인 독립성은 보통 독립성이 아니다.**
   그래서 공짜로 얻던 것을 **완료 조건으로 바꾼다**: M1과 M2는 사서를 끈 상태의 통과를 각각 1회 요구한다(위 표). MCP 서버를 내리고 파이프라인을 한 번 더 돌려, 카드가 `degraded`를 달고 채우지 못한 값을 `assumed`(E5)로 올린 채 검증기를 통과해야 한다. 이것은 원래 순서보다 **오히려 강하다** — 원래 순서에서는 축소 모드가 검증되고 정상 모드가 검증되지 않았다.
2. **M1–M2에 쌓인 E5를 나중에 되짚어 보는 실험이 사라진다.** 원래 계획의 "사서가 생기는 순간 그 E5 중 무엇이 실은 문헌에 있었는지가 드러난다"는 서비스의 값을 재는 좋은 측정이었는데, 사서가 먼저 있으면 되짚을 더미가 쌓이지 않는다. 이것은 대체하지 않는다. 잃는다.

따라오는 결과 셋:
1. **E5 상한(§11-2)은 여전히 미결이다.** 재배열로 gap 탐지가 먼저 생기지만 그것만으로는 부족하다 — 상한은 "정상 운영에서 계획 하나에 추정이 몇 개 나오는가"의 표본을 요구하고, **계획이 아직 한 장도 없다.** 조건은 둘이다: gap 탐지가 켜져 있고, 그 상태에서 만들어진 계획이 표본이 될 만큼 쌓여 있다. 그때까지 검사 3은 `UNDECIDED`로 남는다 — 아무도 고르지 않은 임계값은 만족된 임계값이 아니다.
2. **`gaps`가 첫 계획부터 채워진다.** 이것이 재배열의 이득이다. 현미경의 첫 goal 카드가 "무엇을 놓쳤는지 모른다"가 아니라 "무엇이 없는지 안다"에서 출발한다.
3. **하드웨어 연결은 각 단계의 마지막 항목이다.** 검증은 mock 백엔드로 하고, 실제 장비와 HOOMD는 mock으로 파이프라인이 통과한 뒤에만 붙인다(§4.6.5).

**저장소와 서비스는 여전히 별개다**(§4.3.0). `kb/`는 M0 시점부터 사람이 큐레이션하는 저장소로 존재해 왔고, 사서를 먼저 세운다는 것은 그 위에 서비스를 올린다는 뜻이다. 저장소만 있던 기간이 짧아졌을 뿐 구분이 없어진 것은 아니다 — 등재는 계속 사람과 사서 세션의 일이고(`curated_by`), 서비스가 자동으로 하는 것은 **질의·`gaps`·충돌 탐지**다.

M1–M3은 각각 **단독으로 쓸 수 있는 에이전트**를 하나씩 완성한다(D5). 순서를 바꿔도 이것은 변하지 않는다 — 바로 그래서 바꿀 수 있었다. 브리지(M4)는 이미 혼자 돌아가는 것들을 잇는 단계이며 앞 단계의 전제가 아니다.

M0이 먼저인 이유: 계약이 없으면 네 에이전트가 각자 다른 숫자 표현을 만들고, 그것을 나중에 맞추는 비용이 전체를 다시 쓰는 비용보다 크다. 실제로 M0을 쓰는 동안 설계 오류 다섯 개가 드러났고(§0.2), 그것이 계약을 먼저 쓴 값이다.
...
1. **관측량 어휘 — 내용만 남았다.** 형태는 확정했다(`contracts/observables.json` + `schemas/observable.schema.json`): 한 항목은 `definition`, 허용 단위, **`estimator`**, `window_required`, `producible_by`, `comparable`을 갖는다. 남은 것은 **이 실험실이 실제로 재는 관측량이 무엇인가**이고, §1이 측정 대상을 열어두기로 했으므로 미리 열거할 수 없다 — 질문이 필요할 때 한 항목씩 자란다.
   ...
   **2026-09-17 부분 확정** — 어휘 전체는 실제 질문이 생길 때까지 열어 두고, `capabilities/microscope.json`은 `status: provisional`로 **구성 다섯 개를 전부 선언하되 관측량은 하나**(`tracer_diffusivity`)만 싣는다. 구성 목록은 추출된 사실이고(§11.1) 어휘는 결정이므로, 둘을 함께 미룰 이유가 없다.
...
2. **E5 상한**: 계획서 하나에 추정값을 몇 개까지 허용할까? 0이면 아무것도 못 하고, 무제한이면 등급 체계가 무의미하다. **조건 두 개가 모두 갖춰진 뒤에 정한다**: ① gap 탐지가 켜져 있고(사서 서비스), ② 그 상태에서 만들어진 계획이 표본이 될 만큼 쌓여 있다. 사서를 먼저 세우면서 ①은 이르게 만족되지만 ②는 아니다 — 계획이 아직 한 장도 없다(§9). 그때까지 검사 3은 `UNDECIDED`다.

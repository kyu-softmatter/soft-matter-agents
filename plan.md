...
| D6 | 내부 파이프라인 | **두 실행 에이전트는 같은 5단 파이프라인을 쓴다**: 질문 정교화(S1–S2, LLM 단독) → **system designer**(S3–S5: 축별 병렬 독립 해석 → 종합·트레이드오프 → 산출). §4.5. |
...
8. **축소 모드의 전파 범위**: `degraded` 표시를 시스템 내부에서만 쓸까, 외부 보고(그림·논문)까지 끌고 갈까?
9. **세션 경계**: 4개 에이전트를 4개 Claude Code 세션으로 항상 분리할까, 한 세션에서 디렉터리 스코프로 전환할까? (권한 격리는 전자가 강하고, 왕복 속도는 후자가 빠르다)

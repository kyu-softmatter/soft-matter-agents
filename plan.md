...
| 도구 | 하는 일 | 누가 쓰나 |
|---|---|---|
| `kb_query(observable, condition_range, purpose)` | 관련 entry + gaps + 등급 | S2, S3 서브에이전트, operator |
| `kb_get(entry_id)` | entry 하나를 원문 그대로 | 누구나 |
| `kb_conflicts(topic)` | 상충하는 주장 쌍 | S3, S4 |

**쓰기 도구는 노출하지 않는다.** 외부 검색, 증류, entry 작성·폐기는 사서 세션 안에서만 일어난다. 서브에이전트가 KB에 쓸 수 있으면 증류되지 않은 값이 곧바로 정본이 된다.

**서버는 무상태(stateless)여야 한다.** 질의 사이에 컨텍스트를 남기면 형제 서브에이전트들이 사서를 경유해 간접적으로 서로를 볼 수 있게 된다 — S3 독립성(§4.5.2)에 뚫리는 구멍이다. 같은 질의에는 같은 답을 주고, 누가 직전에 무엇을 물었는지 기억하지 않는다.
...
4. **정밀도 모드 판정**: 이 질문이 탐색인지 확인인지 정해 `intent`에 적는다. 기본값은 탐색이다(§5.8).
5. **저장**: 사람용 MD + 정본 JSON, 같은 `qid`.
...
goal 카드는 `intent: explore | confirm`을 갖는다. S2가 정하고 사람이 덮어쓸 수 있다.
...
1. **사서 MCP 도구 목록의 확정**: `kb_query / kb_get / kb_conflicts` 셋으로 충분한가? 조건 범위 질의의 인자 모양은 어떻게 잡을까(§4.3.1).

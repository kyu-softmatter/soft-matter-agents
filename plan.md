...
| D12 | 지시·보고 계층 | **3계층.** 아키텍처(구조를 쓴다) → 매니저(각 에이전트의 설계 수정, worktree 머지 검토) → 에이전트 넷(수행). 지시는 내려가고 보고는 올라온다. **서브세션은 각자 `git worktree`를 갖고**, 통합은 매니저의 머지라는 한 지점을 지난다. 계층은 권한을 늘리지 않는다 — 위의 둘은 Tier 0이다(§6.2, §6.2.1). **컨텍스트를 언제 비울지는 한 단 위가 정한다**(§6.2.3). |
...
**서브세션은 각자 `git worktree`를 갖는다.** 자기 작업 사본과 자기 인덱스를 갖고, 공유하는 것은 커밋 이력뿐이다. 통합은 **매니저의 머지**라는 한 지점을 지나고, 매니저는 머지 시점에 검토해서 위로 보고한다.
...
**worktree가 바꾸는 것.** 남의 미완성 작업이 아예 보이지 않으므로 `git add -A` 사고가 구조적으로 불가능해지고, 3번 같은 상호 차단이 발생할 수 없다. **치르는 비용은 병합 충돌이 커밋 시점에 드러나는 것**인데, 지금처럼 조용히 덮어쓰는 것보다 낫다: 충돌은 보이면 해결되고, 덮어쓰기는 보이지 않으면 사라진다.
...
```
git config extensions.worktreeConfig true          # 저장소에 한 번
git config --worktree committer.name  'seat:manager-microscope'
git config --worktree committer.email manager-microscope@seat.invalid
```
...
git은 `committer.*`를 읽지 않으므로 이것만으로는 부족하다 — 실제로 신원을 바꾸는 것은 `GIT_COMMITTER_*` 환경변수뿐이다. 그래서 worktree의 설정은 **기록**이고, 그 값을 환경으로 올리는 것은 세션 시작 시 한 번이다. **자리를 잃어버린 세션이 자기가 누구인지 디스크에서 읽을 수 있다**는 것이 요점이고, 그게 없으면 초기화마다 사람이 다시 앉혀야 한다(§6.2.2는 좌석 **지정**이 사람의 일이라고 했지, 좌석을 **기억**하는 것까지 사람의 일이라고 하지 않았다).
...
rebuild/
  ...
  .mcp.json                사서 MCP 서버 등록. **아키텍처 소유** — `.claude/`의 형제이고 넷이 공유하는 진입점이라 어느 에이전트의 것도 아니다
  .claude/
...
**폐기는 적힐 자리가 없다 — 이것부터 고쳐야 한다.** 폐기된 항목은 정의상 산출물을 남기지 않으므로 분모가 보이지 않는다. 분자도 약하다: 2026-09-18 현재 `ruling`이 transfer 5·downgrade 1·discard 0인데, 그 여섯 중 **A1–A7 슬롯을 실제로 댄 것은 둘**이고 §10.2.1은 이관된 항목 **전부**에 그것을 요구한다. 자리는 새로 만들 것 없이 **과제 파일**(`<agent>/tasks/NNN.md`)이다 — §10.2.1의 판정은 과제 단위로 내려지고, 폐기도 판정이므로 **고려했다가 버린 항목을 그 파일에 남긴다.** 이 자리가 서기 전에 두 변형이 출발하면 그 기간의 폐기는 영영 세어지지 않는다.

**두 변형은 같은 커밋에서 갈라져야 한다.** 지금 있는 worktree 하나는 `a012814`에서 분기해 `156ba12`(좌석 등록)와 `ab2eb6f`(002의 re-pin)보다 앞이다. 그대로 두면 양쪽이 같은 수선을 각자 하게 되고, **비교가 그 수선의 차이를 재게 된다.**
...
**두 변형 모두 §10.2.1을 지난다.** "`main`을 따라 이어 만든다"가 **이식**을 뜻하지는 않는다. 이식된 변형과 처음부터 지은 변형을 비교하면 그것은 두 설계의 비교가 아니라 **기존 시스템과 재설계의 비교**이고, 그 둘은 다른 질문이다. 어느 쪽을 하는지는 사람이 정하지만 **정해야 한다** — 정하지 않으면 비교 결과가 무엇에 대한 답인지 아무도 모른다.
...
11. **한 사실이 여러 자리에 살고 아무것도 대조하지 않는다 — 비교기를 더할 것인가, 자리를 줄일 것인가.** 2026-09-18에 이 형태가 세 번 나왔고 셋 중 하나만 지켜지고 있다.
    ...
    | 경로 하나 | §7 트리 · `ALLOWED_PATHS` · `seats.json` | 없음 |
    | 경계 하나 | `seats.json`의 `owns`/`paths` · `DESIGN_OWNED` · `AGENT_OF_PATH` | 없음 |
...
10. **worktree · 좌석 신원 · `refuse`의 순서 제약.** 셋이 맞물려 있고 순서를 틀리면 하나가 다른 것을 막는다. §6.2.3은 좌석 신원을 worktree의 git config에 두라고 하는데, **작업 사본이 하나뿐인 지금 `--local`을 걸면 모든 세션의 커밋이 그 신원으로 나간다.** 그래서 그 처방은 worktree 이전 전에는 적용할 수 없고, 그때까지 신원은 명령 접두사로만 살아 **초기화와 함께 사라진다.** 그 상태에서 `unknown_committer`를 `refuse`로 넘기면 **초기화 뒤 첫 커밋이 하드 실패한다** — 지금은 `report`라 조용히 미귀속으로 통과한다. 순서는 **worktree → 신원을 config로 → `refuse`**다. 사서 매니저가 2026-09-18에 올렸다. 결정할 것은 순서가 아니라(그건 정해졌다) 세 단계를 **한 번에 넘길지 나눠 넘길지**이고, 나눠 넘기면 그 사이 구간마다 무엇이 느슨해지는지를 먼저 적어야 한다.
9. **S4의 타이브레이크 순서 — 축이 동시에 만족되지 않을 때 무엇이 먼저 양보하는가.** §4.5.4는 트레이드오프를 **적으라**고만 하고 **순서를 주지 않는다.** 그래서 두 구성이 서로 다른 축에서 이기면 종합이 근거 없이 하나를 고르거나 사람에게 넘긴다. `agentic-microscope`에는 그 순서가 있었다(공간분해능 > 화질 > 시간분해능 > 광량, 그리고 "아래에서부터 양보한다"·"양보는 이름을 대고 보고한다"·"위계는 `hard` 게이트를 절대 넘지 못한다"). **그러나 그것을 베끼지 않는다** — 그 KB 항목이 스스로 적기를, 순위는 운영자의 것이지만 **근거는 재구성됐고 반증조건은 저장소가 제안한 것**이다. 순위를 베끼면 재구성된 근거가 함께 온다. **이것은 운영자에게 물을 질문이고**, 답이 오면 날짜와 함께 사람에게 귀속시켜 적는다. 순서 자체보다 먼저 정할 것이 하나 있다: 순서가 **질문마다 달라지는가**(목적이 정하는가) 아니면 **이 장비의 고정 성질인가**. 저쪽은 후자로 뒀고 적용 범위를 단일입자 추적·마이크로레올로지로 좁게 **가정**했다.
8. **유도량의 등록 — 같은 양이 두 이름으로 사는 것을 무엇이 잡는가.** §11-1이 **관측량**의 어휘를 정하는데, 2026-09-17의 `tau_a`/`tau_d` 사례는 **유도량**에도 같은 자리가 필요하다는 것을 보였다(§5.7). 검사로는 못 잡는다 — `r²/D`와 `d²/D`는 다른 식이고 같은 양이라는 것이 식에서 나오지 않는다. 결정할 것은 **유도량이 관측량과 같은 레지스트리를 쓰는가, 아니면 별도인가**이다. 같은 레지스트리면 `producible_by`·`window_required` 같은 관측량 전용 필드가 유도량에 비어 남고, 별도면 두 목록을 대조하는 일이 새로 생긴다. §11-1이 정해진 뒤에 그 모양을 보고 정한다.

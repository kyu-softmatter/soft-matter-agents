...
- 재실행은 qid를 재사용하고 `revision`을 올린다. 이전 산출물은 지우지 않고 `r2_` 접두사로 나란히 둔다(P9, **§7.1 규칙 3**).
...
**격리되는 것은 worktree가 아니라 거기서 열린 세션이다.** 다른 자리에 앉은 세션이 Bash로 worktree를 **원격 조종**하면 세 겹 중 둘이 안 걸린다. 2026-09-18에 확인했다:

| 겹 | 원격 조종에서 |
|---|---|
| `.claude/settings.json`의 deny | **안 걸린다** — 패턴이 `Write(microscope_agent/**)`처럼 상대이고 세션의 루트 기준으로 풀린다 |
| PostToolUse 훅의 검증기 | **딴 트리를 본다** — `$CLAUDE_PROJECT_DIR`가 세션의 루트이지 쓴 트리가 아니다 |
| 커밋 게이트 | **걸린다** — `core.hooksPath`가 저장소 공용이고 만드는 트리를 본다 |

**하필 성한 하나가 가장 늦은 것이다.** 쓰기 시점에 막는 둘이 죽고 커밋 시점에 막는 하나가 산다. §6.2가 "잘못 앉은 세션은 조용히 넘어가는 대신 시끄럽게 막힌다"고 적은 성질이 원격 조종에서는 뒤집혀서 — **쓰는 동안은 조용하고 커밋에서만 시끄럽다.** 사서 실행석1이 009를 그렇게 처리할 수 있었는데 판단으로 피하고 올렸다. 규칙은 하나다: **worktree에서 할 일은 그 worktree에서 열린 세션이 한다.**

...
**그런데 이것을 잡은 검사가 없었고, 어느 검사도 게을러서가 아니었다.** 검사 36은 **무차원군만** 본다 — `tau_a`는 시간이므로 시야 밖이다. 검사 39는 `assumed:` 숫자가 gap을 **가리키는지**는 보지만 **그 gap이 진짜인지는 보지 않는다.** 그래서 카드는 없는 구멍을 선언하고 통과했다. 이름이 달라서 비껴간 것이 아니라, **이름이 다른 두 양이 같은 양인지를 판정하는 자리가 아예 없었다.**

세 가지가 다르고, 섞으면 안 된다:

1. **거짓 gap은 검사할 수 있다.** `gap_id`가 KB에 실제로 있는 entry를 가리키면 그것은 gap이 아니다. 지금 걸린 사례를 잡지는 못하지만(이름이 다르다) 가장 값싼 절반이고, 검사 39가 이미 gap 목록을 읽고 있으므로 자리가 있다.

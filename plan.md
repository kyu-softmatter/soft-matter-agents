...
**그리고 선언과 통과 사이에 창이 열려 있다**: `axis.schema.json`의 패턴이 아직 축 형태만 받으므로 `s2`·`operator`·`bridge:…`는 **선언돼 있고 서버가 거절한다.** 형식이 이 절의 것이라 선언이 먼저일 수밖에 없었고, 매니저의 스키마 변경으로 닫힌다. **거절된 호출은 로그에 줄도 남기지 않으므로** 그동안 그 자리들의 질의는 흔적조차 없다 — 창이 열려 있다는 것을 아는 것이 그래서 중요하다.
...
caller_id = <qid>:v<N>:s2                  예) mic-20260916-001:v1:s2
          ...
          | bridge:<thread>:r<N>          예) bridge:mic-sim-0001:r2
```
...
| 46 | `result`의 어휘 핀이 해소되는지(현재 도출값이거나 이력의 커밋) | manager-bridge |
...
**2026-09-19에 처음 충족됐다.** `microscope_agent/questions/mic-20260918-001/axis_widefield_inline_a4.json`이 `kb_refs` 다섯 · `kb_gaps` 다섯 · `degraded: []`이고, 그 `caller_id`가 `queries/log.jsonl`의 기록으로 뒷받침된다. **gap 다섯이 전부 `absent`인 것이 이 통과의 성격을 말한다** — 서비스가 답했다는 것이 "다 찾았다"가 아니라 **무엇이 없는지를 서비스가 말해 줬다**는 뜻이고, 그것이 §4.3.1이 `kb_gaps`를 둔 이유다. 파일을 직접 읽었으면 그 다섯은 아무 데도 기록되지 않았다.
...
12. **`kb_query`의 인자가 `observable`인 것이 맞는가.** 2026-09-19 서비스 첫 실사용에서 `kb_query`가 열한 번 불렸는데 **등재된 관측량으로 물은 것은 0건**이었다 — 표 이름 하나, 열 이름 둘, entry_id 여럿. entry는 이미 `subject`로 네 kind(device·configuration·observable·quantity)를 구분하는데(검사 44), **질의는 그중 한 kind의 이름을 인자로 쓴다.** 그래서 호출자가 장치 사실을 물을 방법이 "관측량 칸에 장치 이름을 적는 것"뿐이고, 로그의 그 칸에 세 종류의 이름이 섞여 **앞으로 쓸 대조 검사(§0.3-4의 `degraded` 대 로그)가 조용히 어긋난다.** **가장 짧은 증거는 두 도구의 성적 차이다**: 같은 로그에서 `kb_get`은 **5/5** 답했고 `kb_query`는 일곱 중 **다섯이 `absent`**였다 — **호출자가 entry id를 알면 찾고 모르면 못 찾는다.** 그리고 그 다섯 중 `automatable_condition`은 **사서가 과제 001에서 `dmd` 행에 직접 지어 쓴 필드 이름**이다. 자기가 발행하는 파일에 있는 자기가 지은 이름을 두고 "없다"고 답했다. 결정할 것: 인자를 `subject`로 넓히고 kind를 함께 받을 것인가, 아니면 도구를 늘릴 것인가. 전자면 §4.3.1의 도구 표와 `query_log.py`의 한 칸이 같이 움직이고, 후자면 "도구 넷과 다섯째는 없다"가 깨진다. **소비자가 생기기 전이 값싸다** — 지금 로그는 열한 줄이고 읽는 것은 사람뿐이다.

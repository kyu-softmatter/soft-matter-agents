...
- **파일 분할 기준은 제어 채널이다.** 본체 SDK 하나가 열 개 요소를 다루면 `dev_body.py` 한 파일이 그 열 개를 맡는다. 요소마다 파일을 쪼개면 같은 SDK 핸들을 여러 파일이 잡게 되고, 그 순간 단일 진입점(§4.6.7)이 깨진다.
...
1. **장치 레지스트리가 envelope의 일부다.** `envelope/devices.json`에 장치마다 다음을 적는다:
   ...
   광경로 제약은 여기가 아니라 `envelope/optical_paths.json`에 있다(§4.6.8).
...
3. **광경로는 쌍 배타가 아니라 상태 벡터다**(§4.6.8). 초기 설계의 `exclusive_with`는 이 장비를 표현하지 못한다 — 아래에서 교체한다.
...
#### 4.6.8 광경로는 상태 벡터다 — 그리고 그것이 곧 "구성"이다
...
| 광경로 락 | `optical_paths.json`(§4.6.8)의 유효 튜플을 **락 그룹으로 컴파일**한다. 전환 중에는 어떤 취득도 시작되지 않는다. |
...
- 광경로 재구성 (`optical_paths.json`의 다른 구성으로 넘어가는 변경, §4.6.8)
- 수동(`manual`) 채널 장치 조작
- 시료에 비가역적인 측정
- 계획에 새 장치가 들어오는 경우
- 물리 모델 변경 (시뮬레이션)
...
rebuild/
  ...
  microscope_agent/                 최상위 폴더 3개 + CLAUDE.md
    ...
    envelope/                       safety.json        안전 정책 — 사람 소유, Tier 3 (§2.1)
                                    optical_paths.json 유효 광경로 = 구성 목록 (§4.6.8)
    ...
    questions/<qid>/                한 질문의 모든 것이 이 한 폴더에 평평하게
                                      ...
                                      goal.json                       (S2, 정본)
                                      axis_A1.json … axis_A7.json     (S3, 축별 제약)
                                      synthesis.json                  (S4)
  ...
  simulation_agent/                 위와 같은 구조, 축 목록만 다름 (§4.5.3)
    ...
    questions/<qid>/                question_…md, goal.json, axis_A1–A7.json,
...
| **M2** | **시뮬레이션 에이전트 (제안 전용 → 스모크런).** 제약 검사 코드 (§10.2에 따라 이전 축 로직 참고 가능) | 목표 1건 → 검증 통과한 plan → 스모크런 → 수렴 증거 |
...
| M1 | 두 KB 구조 대조 | 구조만 참고. entry는 출처 검증 후 개별 이관(§10.3). |
| M2 | **시뮬레이션 축 계산 로직** — 안정성·표본화·수렴 판정 | 식과 판정 기준을 참고하되, 우리 축 분해(§4.5.3)에 맞춰 다시 배치한다. |
| M3 | **하드웨어 제어 경로와 장치 스펙** — 드라이버 호출 방식, NA·배율·픽셀 크기·출력 한계 등 구체값 | 제어 경로는 `src/devices/`의 네 함수에 맞춰 다시 감싼다. **수치는 전부 §10.3을 통과해야 한다.** |
| M3 | **현미경 축 계산 로직** — SNR, 광손상, 분해능, 이탈 조건 | 위와 같음. A7의 이탈 조건처럼 이미 검증된 식이 있으면 그것을 쓴다 |
...
6. **S3 축 목록의 확정**: §4.5.3의 7개 축은 제안이다. 새 축을 더할 때는 "상대의 변수를 몰라도 구간을 낼 수 있는가" 기준을 통과해야 한다(§4.5.3).
7. **orchestrator의 병렬 단위**: 장치 단위로 병렬화할까, 작업(모듈) 단위로 할까? 카메라 2대를 한 워커로 볼지 두 워커로 볼지가 여기서 갈린다.
8. **모듈 분해와 backend 인터페이스**: `preflight/apply/read/abort` 네 함수로 HOOMD-blue 호출과 아홉 종 장치 제어를 모두 덮을 수 있는가? 부족하면 무엇이 빠지는가.
9. **장치별 자동화 가능 여부**: 아홉 종 장치 중 SDK/DAQ로 자동화되는 것, GUI 수동인 것, 상태를 되읽을 수 없는 것의 구분. `envelope/devices.json`의 초기 내용이 여기서 나온다(§4.6.6).
10. **유효 광경로 목록**: 열네 개 남짓한 선택자의 조합 중 실제로 빛이 검출기까지 도달하는 튜플은 무엇인가. 이 목록이 곧 구성(모달리티) 목록이고, S3.0·A4·orchestrator 락이 모두 이것을 본다(§4.6.8). **M3 이전에 이것부터 채워야 한다.**

...
rebuild/
  ...
  .claude/                 settings every seat at the root shares, and
                           ...
                           when a session will have no librarian tools (11-18)
...
|---|---|---|
| 77 | whether a trajectory deletion's `triggered_by` resolves -- `declared_in` names a plan card in the tree, the `id` is declared in it, `what` is actually gone, and for `kind: criterion` the run's result card shows that criterion **fired**, which is `met: false` for a **success** criterion and `met: true` for a **stop** one -- this row said `met: false` for both until 2026-09-22 and was exactly inverted on stop criteria. For `kind: falsifier`, the `recorded_in` path it names resolves in the tree | manager-simulation |
...
| 78 | whether **every path the history has touched since the enforcement line classifies into a boundary some seat owns** -- check 55's reverse direction, and **not** a claim that `ALLOWED_PATHS` and `SHARED_PATHS` agree. They must keep disagreeing; only the permitted direction is fixed. A path the classifier knows and disk does not is normal (`plan_ko.md`, which checks 35 and 41 still walk in history); a path that may exist and classifies nowhere is **a file no seat can commit**, which is what `pyproject.toml` and `uv.lock` were on 2026-09-19. **History-wide and not live-tree**, which manager-simulation measured and which decided the form: the live-tree version passes when `plan_ko\.md` is removed from `SHARED_PATHS`, because that file is not on disk, and the history version reports 1. **Scope starts at the enforcement line and what precedes it is out of scope rather than PENDING** -- PENDING names an artifact a later milestone produces, and no milestone makes a commit from before the gate attributable. 673 paths over all history and 655 since `f971c40`, `unattributable` 0 in both today | manager-simulation |
| 79 | whether **every directory `.gitignore` excludes wholesale is in the validator's `SKIP_DIRS`**. The two lists answer one question -- what is not this repository's content -- and live in two files with two owners, and nothing compares them. On 2026-09-23 `.pixi/` was in the first and not the second, and one `pixi install` put 271 MB in the shared working copy and made check 13 report **10757 failures** for every seat at once. The narrow form is deliberate: `.gitignore` also holds file patterns (`*.pyc`, `.DS_Store`) that have no business in a directory-skip list, so the check reads only entries ending in `/` | manager-simulation |

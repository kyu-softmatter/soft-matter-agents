...
|---|---|---|
| 75 | whether a bench-source entry says what ends it -- an event, or that no event does and why, or that it is not known. Advisory while the backlog stands, enforcing at zero, the shape check 29 took for `task`/`occasion` | manager-librarian |

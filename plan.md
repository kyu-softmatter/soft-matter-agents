...
rebuild/
  ...
  microscope_agent/                 최상위 폴더 3개 + CLAUDE.md
    ...
    inbox/<thread>/                 **브리지가 쓰고 이 에이전트가 읽는다.** 배달된 `r<N>_ask_*.{json,md}`와
                                    그 라운드의 `status.json` 사본. **경계는 `bridge`다** — 자리만 이 트리
                                    안이고 쓰는 주체는 브리지다(§6.2-3, §7.1 규칙 8)

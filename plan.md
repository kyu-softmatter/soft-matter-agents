...
|---|---|---|
| 77 | whether a trajectory deletion's `triggered_by` resolves -- `declared_in` names a plan card in the tree, the `id` is declared in it, `what` is actually gone, and for `kind: criterion` the run's result card records that criterion `met: false`. For `kind: falsifier`, the record it names as having fired resolves | manager-simulation |

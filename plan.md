...
**브리지 세션에는 장비 도구가 없고 런 디렉터리 쓰기도 거부된다** — 아무것도 실행하지 않아야 하므로 실행할 수 없어야 한다. **도구 거부는 경로 거부와 달리 불활성이 아니다**: 도구 이름은 루트에 대해 풀리지 않는다. 이 구별이 이 절에서 가장 중요하다 — 같은 파일 안에서 한 종류는 집행하고 다른 종류는 하지 않는다.

...
6. **반복되면 사람을 부른다**: 한 스레드에서 같은 `(reason_code, parameter)` 쌍이 **두 번** 나오면 라운드를 더 열지 않고 사람에게 올린다. **고정 라운드 상한은 두지 않는다** — 숫자는 진짜 진전이 있는 왕복까지 자르고, 제자리를 도는 왕복은 그 숫자에 닿기 전에 이미 낭비다. A→B→A→B 같은 순환도 쌍의 반복으로 잡힌다.
...
**폴더는 성장하는 것에만 준다.** `devices/`가 유일한 서브폴더인 이유는 장치가 교체·추가되며 계속 늘어나는 유일한 축이기 때문이다. `axis_*.py`는 7개로 고정이므로(§4.5.3) 폴더를 주면 빈 계층만 생긴다. 파일이 20개인 평평한 `src/`는 접두사(`dev_`, `axis_`)로 충분히 읽힌다.
...
| **3** | **M2** | **시뮬레이션 에이전트.** 같은 5단 파이프라인, 축 A1–A5, HOOMD 백엔드 | 목표 1건 → 검증 통과한 plan → 스모크런 → 수렴 증거. **사서를 끈 상태로 1회**는 M1과 같이 요구된다. |

...
3. **라운드 상한**: 브리지 왕복 몇 라운드에서 사람을 강제 호출할까?
4. **S3 축 목록의 확정**: §4.5.3의 7개 축은 제안이다. 새 축을 더할 때는 "상대의 변수를 몰라도 구간을 낼 수 있는가" 기준을 통과해야 한다(§4.5.3).
5. **모듈 분해와 backend 인터페이스**: `preflight/apply/read/abort` 네 함수로 HOOMD-blue 호출과 아홉 종 장치 제어를 모두 덮을 수 있는가? 부족하면 무엇이 빠지는가. M1에서 실제로 감싸 보면 답이 나온다.
6. **사서 MCP 도구 목록의 확정**: `kb_query / kb_get / kb_conflicts` 셋으로 충분한가? 조건 범위 질의의 인자 모양은 어떻게 잡을까(§4.3.1). M3 과제다.
7. **축소 모드의 전파 범위**: `degraded` 표시를 시스템 내부에서만 쓸까, 외부 보고(그림·논문)까지 끌고 갈까?

### 11.1 미결이 아니라 M1의 추출 과제

다음 둘은 "정해야 할 것"이 아니라 **이전 저장소에서 꺼내 올 것**이다(§10.2 M1 행, §10.3):

- **장치별 자동화 가능 여부** — 아홉 종 장치 중 SDK/DAQ로 자동화되는 것, GUI 수동인 것, 상태를 되읽을 수 없는 것. → `envelope/devices.json`
- **유효 광경로 목록** — 선택자 열넷의 조합 중 실제로 빛이 검출기까지 도달하는 튜플. 이 목록이 곧 구성(모달리티) 목록이므로 `contracts/capabilities/microscope.json`도 여기서 함께 채워진다. → `envelope/optical_paths.json`

추출한 수치는 등급 상한 E3이고(§10.3 규칙 1), **안전 한계는 이관하지 않는다**(규칙 4).

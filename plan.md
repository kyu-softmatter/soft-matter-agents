...
rebuild/
  ...
  contracts/               ★ 유일한 공유 코드. 4 에이전트가 모두 의존.
    ...
    units.md               양별 허용 단위 + 무차원군 entry의 필수 형식 (§5.7)
...
**현미경이 먼저라는 것은 사서 없이 시작한다는 뜻이다.** M1–M2 동안 모든 카드는 `degraded: [librarian_agent]`를 달고 돌고, 문헌값(E3)을 당길 수 없으므로 E5(추정)가 많아진다. 이것은 부작용이 아니라 **D5·P11(독립 구동)이 처음부터 강제로 검증된다**는 뜻이다 — 나중에 붙이는 독립성은 대개 독립적이지 않다.

따라오는 결과 셋:
1. **E5 상한(§11-3)은 M3 이후에 정한다.** 사서가 없는 동안의 E5 개수는 정상 운영의 표본이 아니다. 그때까지 검사 3은 `UNDECIDED`로 남는다 — 아무도 고르지 않은 임계값은 만족된 임계값이 아니다.
2. **`kb_refs`는 M3까지 비어 있고 검사 25는 N/A다.** 사서가 붙는 순간 이 검사가 살아나면서, 그 전에 쌓인 E5 중 무엇이 실은 문헌에 있었는지가 드러난다.
...
7. **모듈 분해와 backend 인터페이스**: `preflight/apply/read/abort` 네 함수로 HOOMD-blue 호출과 아홉 종 장치 제어를 모두 덮을 수 있는가? 부족하면 무엇이 빠지는가.
8. **장치별 자동화 가능 여부**: 아홉 종 장치 중 SDK/DAQ로 자동화되는 것, GUI 수동인 것, 상태를 되읽을 수 없는 것의 구분. `envelope/devices.json`의 초기 내용이 여기서 나온다(§4.6.6).
9. **유효 광경로 목록**: 열네 개 남짓한 선택자의 조합 중 실제로 빛이 검출기까지 도달하는 튜플은 무엇인가. 이 목록이 곧 구성(모달리티) 목록이고, S3.0·A4·orchestrator 락이 모두 이것을 본다(§4.6.7). **M3 이전에 이것부터 채워야 한다.**
10. **`capabilities/`의 초기 내용**: 이 장비로 가능한 구성(모달리티) 목록을 어디까지 선언할까? §4.6.7의 유효 광경로 목록과 같은 표이므로 함께 채운다. (무차원군 목록은 고정하지 않기로 했다 — §5.7)
11. **축소 모드의 전파 범위**: `degraded` 표시를 시스템 내부에서만 쓸까, 외부 보고(그림·논문)까지 끌고 갈까?

...
**선언이 먼저다.** 선언만 있으면 검사 42는 "declared but not implemented"로 실패하고, 그 실패는 **다음 커밋 하나로 닫힌다** — 기다릴 대상이 자기 자리에 있다. 구현이 먼저면 검사 42는 "implemented but not declared"로 실패하고, 그 실패는 **다른 좌석이 움직일 때까지 닫히지 않는다.** 붉은 시간의 길이가 자기 손에 있는 쪽을 고른다. 그리고 순서가 뒤집히면 압력이 우회로 간다 — 남을 기다리며 붉은 트리를 들고 있는 좌석은 `--no-verify`를 쓴다(§6.2.1이 기록한 그 경로).

따라서 검사를 하나 더하는 절차는 이렇다: 아키텍처가 §8에 선언을 넣어 커밋한다(검사 42가 붉어진다) → 매니저가 구현과 등록을 넣어 커밋한다(닫힌다). 매니저가 먼저 발견한 검사라면 **문안을 아키텍처에 올리고** 그 순서로 돈다. 사서 좌석이 검사 43을 넣던 오후에는 좌석이 하나여서 이 문제가 없었다.

...
- 세션 시작 hook이 주입하는 `~/.claude/knowledge/`(위 두 저장소의 읽기 전용 미러)도 같은 금지 대상이다 — 내용도, **파일명도** 이 설계에 쓰지 않는다. 규칙은 `CLAUDE.md`에 있다.
...
2. **`assumed` 상한**: 계획서 하나에 가정 몇 개까지 허용할까? 0이면 아무것도 못 하고, 무제한이면 P2가 무의미하다.
3. **사서의 외부 검색 경계**: 어떤 소스를 신뢰 목록에 넣을까? 벤더 스펙의 신뢰도 위치는?
4. **라운드 상한**: 브리지 왕복 몇 라운드에서 사람을 강제 호출할까?
5. **S3 축 목록의 확정**: §4.5.3의 5개 축은 제안이다. 이대로 고정할지, 다른 축을 더할지.
6. **orchestrator의 병렬 단위**: 장치 단위로 병렬화할까, 작업(모듈) 단위로 할까? 카메라 2대를 한 워커로 볼지 두 워커로 볼지가 여기서 갈린다.
7. **모듈 분해와 backend 인터페이스**: `preflight/apply/read/abort` 네 함수로 HOOMD-blue 호출과 여덟 장치 제어를 모두 덮을 수 있는가? 부족하면 무엇이 빠지는가.
8. **장치별 자동화 가능 여부**: 여덟 장치 중 SDK/DAQ로 자동화되는 것, GUI 수동인 것, 상태를 되읽을 수 없는 것의 구분. `envelope/devices.json`의 초기 내용이 여기서 나온다(§4.6.6).
9. **광경로 배타 관계**: 어느 장치 조합이 동시에 성립하지 않는가. 이것이 S3 축 A4와 orchestrator 락 그룹의 실질 내용이다(§4.6.7).
10. **측정 대상과 무차원군 목록**: §5.7의 네 개(`k*`, `τ_D`, `τ_k/τ_D`, `f_wall`)는 광트랩 콜로이드 측정을 가정한 것이다. 실제 측정 대상이 무엇인지에 따라 이 목록을 확정하거나 교체해야 한다.
11. **축소 모드의 전파 범위**: `degraded` 표시를 시스템 내부에서만 쓸까, 외부 보고(그림·논문)까지 끌고 갈까?
12. **세션 경계**: 4개 에이전트를 4개 Claude Code 세션으로 항상 분리할까, 한 세션에서 디렉터리 스코프로 전환할까? (권한 격리는 전자가 강하고, 왕복 속도는 후자가 빠르다)

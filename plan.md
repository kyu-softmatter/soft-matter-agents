...
1. **장치 레지스트리가 envelope의 일부다.** `envelope/devices.json`에 장치마다 다음을 적는다:
   `{id, elements: [...], control: sdk | daq | gui_manual | file_watch, automatable: full | partial | none, sync: hw_trigger | sw_sequence | none, limits: {...}}`
   광경로 제약은 여기가 아니라 `envelope/optical_paths.json`에 있다(§4.6.7).
...
그래서 `envelope/optical_paths.json`에 **유효 광경로의 화이트리스트**를 둔다:
...
**그리고 이 항목 하나가 곧 §4.5.3의 "구성(모달리티)"이다.** `contracts/capabilities/`가 `관측량 → 구성`을 매핑하고, `envelope/optical_paths.json`이 `구성 → 선택자 상태`를 매핑한다. **S3.0이 후보를 고를 때 보는 표와 orchestrator가 인터록에 쓰는 표가 같은 테이블이다.** 둘로 두면 언젠가 갈라지고, 갈라지는 순간 "계획상 가능한데 실제로는 빛이 안 오는" 구성이 생긴다.
...
| 광경로 락 | `optical_paths.json`(§4.6.7)의 유효 튜플을 **락 그룹으로 컴파일**한다. 전환 중에는 어떤 취득도 시작되지 않는다. |
...
- 광경로 재구성 (`optical_paths.json`의 다른 구성으로 넘어가는 변경, §4.6.7)
- 수동(`manual`) 채널 장치 조작
- 시료에 비가역적인 측정
- 계획에 새 장치가 들어오는 경우
- 물리 모델 변경 (시뮬레이션)
...
rebuild/
  ...
  microscope_agent/                 최상위 폴더 3개 + CLAUDE.md
    ...
    envelope/                       safety.json        안전 정책 — 사람 소유, Tier 3 (§2.1)
                                    optical_paths.json 유효 광경로 = 구성 목록 (§4.6.7)
...
| **M1** | **현미경 에이전트.** S2 정교화 → S3.0 스크리닝 → 축별 해석 → S4 종합 → S5 산출 → 승인 → 실행. `envelope/safety.json`과 `optical_paths.json` 기계 판독화, 승인 게이트 hook | 승인 없이 Tier 2 호출이 **실제로 차단**된다. mock 백엔드로 파이프라인 1회, 그다음 실측 1건이 편차 기록과 함께 남는다. |
...
| M1 | **`envelope/devices.json`과 `envelope/optical_paths.json`의 초기 내용** — 장치별 제어 채널·자동화 가능 여부·되읽기 가능 여부, 그리고 실제로 빛이 검출기까지 도달하는 선택자 조합 | 이전 저장소에서 **추출**한다. 추측으로 채우지 않는다. 추출한 값도 §10.3 규칙 1에 따라 등급 상한 E3이며, 실제 장비에서 확인되면 E1로 승급한다. **안전 한계는 §10.3 규칙 4에 따라 이관 대상이 아니다** — 사람이 직접 쓴다. |
...
1. **관측량 어휘**: 실험과 시뮬레이션이 공유할 관측량의 표준 이름과 정의를 어디까지 고정할까? (`capabilities/`의 범위, S4를 좌우한다)

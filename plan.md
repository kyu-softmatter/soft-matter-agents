...
**On 2026-09-20 both halves of that were demonstrated in one settings file, and the second half came out backwards.** simulation-2 wrote to `envelope/safety.json` ten times through a python heredoc past a `Write(envelope/safety.json)` and an `Edit(...)` deny, which is the sentence above confirmed rather than news. What is news is the converse: a `Write(tasks/**)` deny **refused a read** -- `sed -n` on `tasks/000-read-this-first.md`, the card that seat is ordered to read first, because the harness files `sed` under writing. `grep`, `cat` and python read the same file. So the list is keyed on **tool names, and a tool name is not an operation**: `sed` reads and is stopped, python writes and is not. It stands orthogonal to what it means to control in both directions at once, and here it made the right thing hard while leaving the wrong thing easy -- the inverse of the sentence above.

**Check 53 cannot see this, and the reason is worth more than the case.** It reads whether a deny entry *starts with* `Bash(` or `Read(`, so an entry spelled `Write(` passes and the check reports that 113 entries block no reading while one of them had just blocked one. This is inside its stated scope, not outside it -- its own note excuses user-level and harness refusals, and this refusal came from a repository settings file. **It audits the spelling of a rule and not its effect**, and effect is not in the tree: the only way to learn what a deny denies is to attempt the operation and be refused. Narrowing the check's patterns treats the case; it does not close the class. Reported by simulation-2, which hit both directions in one day.

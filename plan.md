...
| 순서 | | 내용 | 완료 조건 |
|---|---|---|---|
| ✔ | **M0** | **계약 먼저.** 카드 스키마 + `units.json`/`units.md` + `validate.py`(검사 38개) + `capabilities/` 골격 | **완료.** 사람이 손으로 쓴 goal → axis → synthesis → plan → 승인 → result 왕복이 검증기를 통과하고(`0 failed`), `rejected/`의 카드 8장이 의도대로 전부 거절된다(`--expect-fail`). 에이전트 코드는 없다. |

...
```
git config --local user.name  kyuhwan
git config --local user.email kyuchoi@stanford.edu
git config --local committer.name  'seat:manager-microscope'
git config --local committer.email manager-microscope@seat.invalid
```
...
rebuild/
  ...
  contracts/               ★ 유일한 공유 코드. 4 에이전트가 모두 의존.
    ...
    librarian_tools.json   사서 MCP 네 도구의 이름·인자 서명 (정본). **서버가 이것을 읽어 등록한다**
                             — 소비자가 생산자의 소스를 읽어야 부를 수 있는 인터페이스는 계약이 아니다 (§4.3.1)
...
| 후보 기준 | 어디서 세나 |
|---|---|
| 게이트 통과 | `0 failed`는 최소 조건이지 변별력이 아니다 |
| 구간을 낸 축 / 기권한 축 | 축 카드의 `verdict` |
| 숫자의 등급 분포 | E5가 몇 개인가(§11-2의 상한과 같은 양) |
| `degraded`가 정직한가 | 질의 로그와 대조(§0.3-4) |
| §10.2.1 이관 항목이 자리를 댔는가 | 댈 수 없어 폐기한 것이 몇인가 |

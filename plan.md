...
**커밋 시점 게이트도 이것은 통과시킨다.** 2026-09-17에 세 번째 사례가 나왔다 — 한 커밋이 두 설계 자리의 `contracts/` 편집을 함께 담았고, hook은 스테이지 집합을 출력했지만 **거절할 근거가 없었다.** 양쪽 파일이 모두 `contracts/`, 즉 같은 설계 자리 경로이기 때문이다. 검사 35는 에이전트 경계를 보므로 설계 자리 둘 사이의 혼합은 **정의상 통과한다.** 게이트가 느슨한 것이 아니라 볼 수 있는 것이 없다.
...
**아직 못 잡는 것도 적어 둔다.** 두 설계 좌석이 같은 신원을 쓰면 검사 41도 그 혼합을 통과시킨다 — `c13e742`가 그것이었다. 분리는 위의 규칙 3이 말한 **경로 분할**을 먼저 요구하고, `seats.json`의 `paths` 필드가 그 자리다. 비어 있는 동안 레지스트리가 그 사실을 적어 둔다. **없는 보장을 코드가 암시하지 않는 것**이 이 항목의 조건이다.

**`unknown_committer`는 `report`로 시작한다.** 등록되지 않은 신원은 거절이 아니라 보고이므로, 신원을 채택하지 않은 좌석이 막히지 않는다. `refuse`로 전환하는 전제는 **모든 좌석이 신원을 채택한 것이고 사람도 포함된다** — 손으로 커밋하는 사람이 첫날에 막히면 그 우회가 습관으로 굳는다. 그래서 `human` 좌석은 git 기본 신원이 아니라 **명시적으로 고르는 신원**이다: 기본값을 레지스트리에 넣으면 모든 세션이 그것을 물려받아 통과하고 검사는 아무 뜻도 없어진다. 그리고 `report`가 치르는 값도 적어 둔다 — 귀속되지 않은 커밋은 **아무 검사도 받지 않는다.** 그 경로가 누구 것이었는지 말해주는 것이 없기 때문이다.
...
38. 한 표 — `contracts/capabilities/`의 구성 id 집합이 광경로 표의 것과 같은지, 각 구성이 그 표의 항목을 가리키는지, 선언된 관측량의 단위가 `units.json`에 있는지, 그리고 **합성을 요구하는 `produces` 항목이 실제로 합성 가능한지** — 지목된 구성이 `perturbation`이고 그쪽 `composes_with`가 이 구성을 되지목하는지 (§4.6.7)
...
41. 좌석 귀속 — 커밋(또는 스테이지된 집합)의 **커미터 신원**이 `contracts/seats.json`의 좌석이고, 변경 경로가 전부 그 좌석이 소유한 경계에 있는지 (§6.2.1). 등록되지 않은 신원은 `unknown_committer`가 `refuse`일 때만 실패한다
...
| 장치 인터페이스 네 함수와 `envelope/` 모양 | **하드웨어 제어 경로와 장치 스펙** — 드라이버 호출 방식, NA·배율·픽셀 크기·출력 한계 등 구체값 | 제어 경로는 `src/devices/`의 네 함수에 맞춰 다시 감싼다. **수치는 전부 §10.3을 통과해야 한다.** |
...
✔는 이미 굳은 것이고, 그 행은 열려 있다. 2026-09-17에 장치 레지스트리·광경로 표·대물렌즈 표를 그 근거로 열었다(§11.1).

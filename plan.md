...
- Language convention: **everything inside the repository is written in English** — code, schemas, comments, commit messages, agent instructions (`CLAUDE.md`, skills).
  **The design document is the one exception, and its shape changed twice in one day (2026-09-20).** It was Korean; then briefly `plan_ko.md` was the record and `plan.md` an English rendering generated from it, bound by a `generated-from: plan_ko.md sha256:<hex>` header that `contracts/hooks/pre-push` recomputed and refused on; **it is now `plan.md` itself, in English, with no rendering and no pair.** The Korean text left version control and stays in history, and `plan_ko.md` is gitignored. What ended the pair was its cost: no generator was ever written, so every edit to the record was two hand-edits plus a recomputed hash, and two architecture seats could not hold that at once. **The paragraph this replaces went on describing the pair as current for the rest of that day**, and the pre-push hook still waits for a header that is never coming -- it printed `plan.md carries no generated-from header` on eight pushes and every one was read past. A reverted decision leaves machinery behind, and the machinery keeps announcing itself in a voice everyone has learned to skip. The hook is `contracts/`, so retiring that branch is a manager's.
...
| D12 | Instruction and reporting tiers | **Three tiers.** Architecture (writes the structure) → manager (revises each agent's design) → the four agents (do the work). Instructions go down and reports come up. A tier does not add permission — the top two are Tier 0 (§6.2, §6.2.1). **One exception, the person's, 2026-09-25**: architecture operates the optical tweezers over their TCP interface for the first double-well run, and nothing else on the bench (§11-23). **Worktrees were introduced on 2026-09-18 and reverted the same day** (§6.2.1). |
...
| **P12** | **The output tree is flat, per question.** Every file of one question in one folder, no per-stage folders, rounds and revisions as filename prefixes (§7.1). | Outputs accumulate without limit. **This column said *and are not deleted (P9)* until 2026-09-22, and P9 does not say that** -- P9 is *logs are append-only, a correction is a new revision, never an overwrite*, which governs **the record** and not the data a record names. Discarding a trajectory while its run log and `trajectory_meta.json` stay is neither a correction nor an overwrite, so no principle forbade it and a cost column asserting a consequence its own citation does not carry did. Found when the person asked to keep trajectories and later delete the bad ones with the reason kept. People look things up by "that question". | Three months later you are grepping through your own experimental record. |
...
11. **No laser line is enabled while the light path can reach the eyepieces** (added 2026-09-24). Before any command that enables a laser line or raises its output, the light path is read back **at that moment**, not taken from an earlier state, since a person at the stand or another seat can change it in between. A read-back that shows a route to the eyepieces refuses, and so does a path that cannot be read (rule 2). Under rule 8 the read-back **can refuse and cannot permit**: a path showing no route to the eyepieces is not grounds to proceed, and the enabling still needs its plan or person-approved command list, its limit and the bench (§6.2.1). **Which path state reaches the eyepieces is established at the bench, recorded, and pinned by the state's integer**, never read from its label, because labels in a loaded configuration are downgraded until confirmed (§10.2.1) and the lab's own labels have been swapped once. The trapping laser is outside what code can hold: its power has no software path, its hand control is the person's, and this rule claims nothing about it. Found missing on the day confocal control was first carded: `manager-microscope-20260924-1` looked for it in this section and in the whole record and it was not there. Enforced first in the confocal wrapper card 036 specifies; until that wrapper exists, no laser line is enabled from software at all.
...
5. **Duplicate blocking**: if the same observable at the same conditions has already made the round trip, replace the new round with a KB reference and record that substitution in `status.json`'s `substitutions`. Opening a round twice on the same (direction, observable) **at conditions that have not moved** is a failure (check 8). **That qualifier was in the first sentence and not the second, and the check implemented the second (2026-09-21).** So a round correcting a source that changed under it was refused as a repeat -- and it is not one: **a repeat re-asks what was answered, and here nothing was answered, the ground moved.** The live case is `thr-tracer-diffusivity-001`, whose r1 pins revision 1 where `max_lag_time` is 2 s while revision 2 says 30 s, the diameter having gone from `assumed:` E5 to the store's E2 and `tau_d` following as d³. **A superseding round is legal**, it names the round it supersedes, and **check 8 recomputes the supersession rather than accepting the claim** -- both ledgers already record the source revision, so "the source moved" is derivable, and a round claiming to supersede one whose revision did not move is refused. A verdict the writer may choose is not a gate, which is the same reason the tie grade is recomputed in check 63. **Amending r1 was never the alternative**: the delivered copy must match by `canon_sha`, the ledger pins revision 1, and P1 makes the delivered file the record that the delivery happened. A round is an event.

   **And once supersession is legal, the receiver needs no new channel: delivering it into the inbox *is* the signal.** Presence is the turn (§7.1 rule 8). Without it there would be no path at all in that direction -- the inbox copy is deliberately immutable and `status.json` is not the receiving agent's to read -- which is check 50's finding one level on: 50 tests that a delivery path has a reader, and nothing tested that a retraction has any path.

   **And the two halves of a comparison arrive on different clocks, which the contract states as a round number (2026-09-22).** `ask.schema.json` says `no_counterpart` is the opening round and that *from round two on it means the comparison was skipped, which is a failure*. **Round number is a proxy and the thing itself is whether a counterpart exists.** A goal card supplies units and no values; only a result supplies values. So round two of a thread can legitimately carry `no_counterpart` for `value_comparison` while `unit_consistency` has everything it needs -- the second round here delivers a goal and there is no measurement on the other side yet. **Keyed on the round, the gate calls a true state a failure**; keyed on whether a counterpart card of the right kind exists, it does not. Found by `bridge-4` sending the first superseding round, and it is the same shape as rule 5's own second sentence dropping the qualifier its first had.

**The two halves of a payload go stale differently, which is why the record length is the one that bit.** A number the store backs repairs itself, because a card queries rather than copies -- the microscope goal carries none of r1's numbers, so its axes ask the librarian and get the E2. A **design parameter** does not: `max_lag_time` is the sender's window, not a fact about the world, so no entry contradicts it and no query corrects it. `manager-microscope` drew that line and it is the one to keep. **Without the librarian this rule does not work** — there is no store to point at, so a bridge in reduced mode reopens the round instead of substituting. `degraded: [librarian_agent]` is where that cost is written.
...
**A default that names a real thing manufactures a claim (2026-09-22).** A run on disk records `backend: mock_backend` and its trajectory hash is HOOMD's -- `simulation-4` proved it by re-running both engines at the same seed and matching hashes, which also shows the two are genuinely independent rather than one wrapping the other. The mechanism is `getattr(backend, "NAME", mock_backend.NAME)`: **a backend without a `NAME` makes the log assert another backend's.** That is P2's error moved into code -- a value with no source, presented as one -- and the schema cannot catch it, because `backend` is a free string whose only guarantee is that it is a string.

    **So the rule is not a check but a refusal to be able to say it: the default is absence, never another name.** A run whose backend cannot identify itself does not write a log claiming one; it fails. A lie that cannot be formed needs no check to find it, and this one **cannot be found later by reading the file** -- every field in it is consistent with every other.

    **Which is the class, and it now has three instances in one line.** *Which plan did this run execute* (fixed 2026-09-21), *which engine ran it* (here), *what did the result card read back* (open, `008`). Each artifact agrees with itself perfectly; **nothing compares any of them to what the execution path actually rested on**, and self-consistency is exactly what makes the set invisible. §4.6.5 defines reproducibility as *the same plan runs unchanged on the mock and on the real instrument* -- **a definition that needs which one ran to be true**, and that is the field that was free to be wrong.

    The mislabelled run is **recorded and not corrected** (P1). It is evidence of the defect, and editing it would remove the only instance anyone can point at.
...
**Carrying is not querying, and the distinction was already built before it was asked about (2026-09-20).** `microscope-1` found that the three bounds S4 has to carry all cite `kb:` sources, that check 54 requires a `kb:` basis to resolve against the **citing** card's `kb_refs`, and that rule 4 means S4 never asked for any of them -- and read it as a contradiction in this document with two ways out, a new carry marker or an exemption in 54. **Neither is needed.** A synthesis card already carries values with `origin: <file>#<name>` on `$defs.number` in `common.schema.json`, and check 12 already verifies them: the run reads *70 carried numbers match their source*. A carried bound is the same act on a different object, so **it takes the same field -- which does not exist on a bound yet.** `origin` is on `$defs.number` alone; `$defs.interval`, `$defs.allowed_set` and `$defs.precondition` are each `additionalProperties: false` with no `origin`, measured 2026-09-20. This paragraph first said the mechanism was already there; **the concept and its check are, the field is half there**, and neither this seat nor the one reporting knew which until somebody ran it. What is settled is the *rule* -- **check 54 resolves a carried `kb:` basis against the `kb_refs` of the card `origin` names**, not an exemption but the right card, since that is the one that did the asking -- and what is left is putting `origin` on a bound. **It goes beside the bound and not inside it**: a closed `$defs` cannot be widened by an `allOf` branch adding properties, which was tried and rejected with *Additional properties are not allowed ('config', 'origin' were unexpected)*, so the shape is a wrapper holding `config`, `origin` and a `bound` that `$ref`s the existing definition. `common.schema.json` is not edited at all.

**Both halves landed on 2026-09-20, an hour apart, and the gap between them has a name worth keeping.** `8bcee35` added `allowed_sets`, `preconditions` and `unbounded` to `synthesis.schema.json` and lifted `bound_state` and `unbound_kind` into `common.schema.json` so the two contracts cannot drift on what `abstained` means; `eb1177a` then changed check 54, which until then still read the citing card's own `kb_refs` and would have failed every correctly carried bound. For that hour the next synthesis card would have reddened the tree, and **a red that is the ruling arriving before its implementation is not a defect in the card** -- the window closed before anything hit it, and the sentence stays because there will be another day when a ruling lands first and someone needs a name for what they are looking at.

**How a carried basis resolves, which the rule above does not settle on its own.** `allowed_sets[].from_axes` and `preconditions[].axis` name the axis that asked, and the test is **at least one of them cites the entry, not all of them**: an `allowed_set` that is two axes intersected draws its basis from whichever axis produced each part, so demanding that every named axis cite every entry rejects bounds that were carried correctly. What it still catches is the case that matters -- **an entry no axis ever obtained**. And an axis absent from the run is reported as absent rather than as not-citing, because the second sends a reader to the wrong file. Verified against seven constructed bundles before a real card existed to try it on, with the 23 existing references still resolving.
...
5. **Different control channels have different failure modes.** An SDK throws an exception, a DAQ **quietly outputs the wrong voltage**, and a GUI tells you nothing. So every device has to be able to read its own state back through `read()`, and **a device that cannot be read back is treated as `automatable: none`.** Automation that cannot be confirmed is not automation. **One exception, and it is the person's** (§4.6.6.1 rule 3, 2026-09-20): `laser_combiner` and `optical_tweezers`, named and no others, are driven by their own wrappers with `verification: none`, and no irreversible action goes through either. Written here on 2026-09-24 because the router implemented this rule without the exception and so kept both channels on the manual sheet -- correctly, until the two rules were made to read as one. A blind channel is routed to its wrapper only by that named exception, never by a registry value alone. **And a blind laser is assumed on**: after any command to it, its state is taken as the most dangerous one the commands allow, until something that can be read back -- a shutter, the light path, the person -- shows the beam blocked (§2.1 rules 2 and 8).

**A safety interlock that resolves a device by guessing at spellings refuses the wrong party (2026-09-22).** The retract interlock does not look its element up in the device registry, which is what this seat was told and relayed on: `manager-microscope` measured the predicate and it is a tuple of hints -- `focus`, `z_drive`, `z_axis`, `objective_z`, `z` -- plus a `startswith("focus")` and an `endswith("_focus")`. **`ZDrive` does not match, and `ZDrive` is the likeliest name the row will carry.** So a correct registry row can land and the interlock still not see it, and the refusal it then prints -- *no element in the device registry drives focus, and the registry is the librarian's* -- **is wrong in every clause while reading as if it were right**, and sends the person to the seat that just did the work correctly. **A refusal that names a cause has to have looked at that cause.** **And the fix is not that the row spells itself to suit the predicate (`librarian-2`, 2026-09-22).** The element landed as `z_drive`, which the predicate happens to match, and that is a **result and not the reason**: element ids in that table are this repository's snake_case -- `nosepiece` for `Nosepiece`, `motor_stage` for `XYStage`, `lapp_branch` for `LappMainBranch1` -- and the device label went where 028 put the others, in `identifiers.mm_label`. **A registry that chooses its spelling to match a caller's string table has already taken the coupling this paragraph objects to**, one level over; the interlock would still be guessing if the convention had produced `ZDrive`. An id that is a registry name sometimes and a device label other times is not an id. The shape that ends it is a **field on the element row saying it drives objective Z**, read rather than recognised; adding `ZDrive` to the tuple is the shortcut and was refused for the right reason -- an unverified relayed name entering a safety path by a side door, which §10.3 cannot screen and P0 keeps a model's value out of.

    **The seat that found it relayed the false half itself, and says so**: it labelled `ZDrive` an unverified quotation and that label travelled correctly; what did not was its own sentence about the code, read off the fact that the function iterates the registry **without running the predicate**. The mechanism sounded right and did not survive being run, which is the shape this document keeps finding, arriving this time inside a safety interlock.

**A plan that runs auto -> manual -> auto re-reads what it handed over (2026-09-22).** The rule already here stops automatic commands to a device family **while** a manual sheet is open. What it does not say is what happens on the far side. `microscope-5`'s two-stage focus hands an encoder Z across an immersion step, and **while the sheet is open nothing stops a person nudging the stage** -- the handed value then goes quietly wrong and the resumed half computes from it. **So a resumption re-reads the state it was handed and compares, rather than trusting it**, and a disagreement outside the plan's tolerance stops rather than proceeds (P0). This is check 66's discipline one layer over: the run reads back whether the limit held, and here the run reads back whether the position held. **Nothing carried across a human's hands is evidence of itself afterwards.**
...
**A fourth means, ranked with the first, for a side that has no clock at all (2026-09-20).** This section was written for cameras and its three means all name hardware or the OS. A simulation run has neither: its time is `steps_taken * dt`, an integer count times a fixed step, and **it reads no clock**. All three values are therefore closed to it -- the first two are false, and the third is honest and **fails check 37**, so the only true answer was the one refused and no result card could reach disk. The fourth is **`model_step_index`**, and it ranks with the first for the first's own reason: *the index is the time and there is no jitter*. A step index has that property exactly, which is why the temptation is to write `trigger_counter` and pass -- **the right verdict for the wrong reason**, since that value names hardware. The seat that found it named the temptation and declined it. `t0_wall` still records when the run started; what changes is only the claim about what the physics rides on. **Check 37 accepts it for a simulation backend and nowhere else**, so an instrument result cannot use it to leave the rule.

**And this is not an exception to the rule below -- it is a case the rule never reached.** The rule forbids computing physics from a *software timestamp*; a step index is not a timestamp. The guard stays live for the simulation side too, and bites the day a run does read a clock.
...
- `degraded`: records that it proceeded without an absent collaborator (§3.1). Empty is normal and non-empty is normal. **Only hiding it is abnormal.**
  ...
  **A card that asked nothing has no absent collaborator, so its `degraded` should be empty -- and today it is not (2026-09-22).** `cards.evidence` defaults to degraded and clears only when a response names the server, so **an axis with nothing to ask can never clear it and is permanently degraded on paper**: three simulation axes say the librarian was unavailable while it answered their three siblings in the same fan-out. False about the run, and the definition above says why -- the field records an **absent** collaborator and nothing was absent. It is `kb_gaps`'s distinction one level up, missing: looked-for-and-absent is not nobody-checked.

  **Three states, and the existing fields already separate them**: asked and answered is `kb_refs` non-empty; asked and unanswered is `degraded` naming the librarian; asked nothing is all three empty with no call in the log. So no new field is wanted. **What would break is check 45's inference** -- it reads an empty `degraded` as a claim the service answered, which held only while every card asked, and it should key on `kb_refs` non-empty instead. The check §8 abandoned -- *if `degraded` is empty then one of `kb_refs` or `kb_gaps` must be filled* -- was declined for catching one card, and the better reason is that it would be **false**.

  **Ruled 2026-09-22: `caller_id` becomes optional in `axis.schema.json`, and that is the whole fix.** The deferral below was written expecting a revision to carry the correction; revision 3 came and **could not**. All three routes are closed by contracts rather than by effort: emptying `degraded` makes check 45 demand a log line under a `caller_id` that never called; dropping the `caller_id` is refused by `axis.schema.json`, which requires it; omitting `degraded` reads as empty because the check does `or []`. **So the count grows -- three false cards per revision, six now** -- and a falsehood that multiplies is not one to leave standing while other things are tidied.

  **The field does two jobs and an axis that asks nothing has neither**: there is no caller to name and no session context to isolate. **Check 45 is already written for this case** -- `if not cid: continue` -- so the schema is what prevents the case existing, and the fix is the schema and not the check, which is `librarian`'s sentence. **The precedent is in the repository**: a `goal` card carries no `caller_id`, keeps `degraded: []`, holds honest `kb_refs`, and check 45 skips it. The shape is legal for a goal and forbidden only for an axis.

  **The deferral below stands for the rest.** **Not fixed today, and the ordering is `simulation-2`'s.**  §9.1's reading closes the milestone question touching no code; changing `cards.evidence` or the schema first would regenerate cards already committed as revision 2 at `946831c`, which is the in-place regeneration `tasks/000` has recorded four times. **So the false `degraded` on three cards stands, written down here, until a revision moves them for a reason of its own.**
...
**Three limits of that, all found by classifying the 49 rather than by arguing about it (2026-09-21).** *`no_event` covers two cases and the field description named only one.* A claim can be timeless -- methodology, not this bench -- **or it can drift**: `lab_ambient_temperature` read at 20 °C starts ageing the moment it is taken and **no event ends it**. Both answer *nothing ends this*, and in the drift case **the `date` beside it is a re-measurement due date and not an expiry**. Saying so here is what stops the next seat inventing `event_unknown`, which is false -- the event is not unknown, there is none. *An ending inside the store is not an expiry.* Two entries end by being **superseded**, and a supersession is §7.1 rule 8's, not `valid_until`'s: **a world-event and a store-event take different fields**, the same line drawn for rename against supersede. *And the prefix sorts an entry only when the entry holds one kind of claim.* Three do not: `trapping_laser_identity` folds *1064 nm, 5 W rated* -- true wherever it sits -- inside *the trapping laser here is a Tweez 300*, which a swap ends. **The camera case worked because `camera_sensor_geometry` and `cameras_both_kinetix22` are two entries**, and the ruling above is only as good as that separation; where a spec is welded into a bench fact the remedy is to split the entry, which is `quantities.json` rule 1's, not this section's. Four more have A and B running through the middle for the same reason.

**And one event does not cover many, which the pixel-size case made look likely.** There twelve entries shared one source; across the 26 assembly-state claims the groups are **nine**, the largest eight. So closing A is nine questions to the person rather than one, which is small enough to be worth asking and is the person's time to spend -- it blocks nothing meanwhile.

**Where no event is known, the entry says so rather than leaving the field out.** Today's 49 are invisible; *no event ends this, because …* is countable and arguable, and a blank is neither. **The live hole is the argument for all of this**: twelve E2 pixel sizes name camera replacement as their terminating event, and `cameras_both_kinetix22` -- the entry binding them to that camera -- has no expiry at all. The chain of validity stops exactly where it is load-bearing, and it took asking the person *what ends this* to see it.

**`within_tolerance` is three claims wearing one boolean, and only one of them is derivable today (2026-09-21).** *Exact reproduction* says the numbers do not differ. *Quantisation* says **the device could not have done otherwise** -- 7 ms to 8 ms is this, not a tolerance. *Declared tolerance* says a difference of this size does not matter. Collapsed into one field nothing follows from it, which is why check 72 returns UNDECIDED rather than FAIL on two deviations: **`within_tolerance` is the only tolerance concept in `contracts/` and nothing anywhere declares a value.** Split, the first is derivable now; the second needs the device's settable grid, which lives in the registry §4.6 gave the librarian as knowledge; the third needs a tolerance in the plan or the envelope. **Until each has a home the split reports what is missing rather than guessing**, and `manager-simulation` was right not to add the field first -- **a slot with nothing to fill it from is one more instance of the class above**, and it declined to create one while reporting the class.

**Every number in a result card is checked and no verdict is (2026-09-20).** Auditing the first result card's contract rather than the card, five required fields turn out to be read by nothing: `criteria_evaluation[].met`, `deviations[].within_tolerance`, `estimation.followed`, `approval_id` and `run_id` -- zero occurrences of each in `contracts/validate.py`. The first three are **the card's conclusions**, and their absence is P2 one level up: P2 exists so a model cannot assert a *number*, and nothing stops it asserting the *judgement drawn from* numbers that are themselves impeccably sourced. A card may carry an `observed_number` that plainly fails its criterion beside `met: true` and pass every check. **The contract verifies the evidence and never the conclusion.** The last two are the chain's last links: `plan_hash` is resolved and `approval_id` and `run_id` are not, so **the approval chain is verified up to the plan and stops before the result** -- nothing confirms the run reported is the run that happened or that the approval named ever existed, which §2.1 rule 7 assumes on the way in. Declared as 72 and 73. Found because `manager-simulation`'s commit message said the contract would not have noticed if the first card were unclean, which is a claim about the contract and so this seat's to check; the card itself was not read.

**A run must be able to say what it read, and on 2026-09-20 it could not (§5.3).** The independence gate refuses `simulated:` for a configuration whose output its input fixes, with the right reason -- the evidence is the input, and citing the run would launder the input's grade. But `values[].number`, `criteria_evaluation[].observed_number` and `deviations[].actual_number` all name entries in `numbers[]`, every entry needs a source, and none of the three kinds fits what an integrator emitted: not `measured:` (a simulation is not a measurement), not `computed:` (it owes a recomputation, check 17, and there is none), not `simulated:` (just refused). **So a verification run of `bd_overdamped` could not state its own observation, and the card could not be written at all** -- which inverts the gate: it exists so a run cannot stand in for evidence, and it was stopping a run from reporting itself. Found by `manager-simulation` trying to write the card, not by reading.

**The ruling: one kind, two roles, and the role is already visible in the field.** `simulated:<run_id>` is legal wherever a number's role is *this is what the run read*, and the independence declaration governs only whether that number may **also** stand as a claim about the world. So the gate keeps refusing it under `values[]`, where the card asserts something about the system, and stops refusing it under `criteria_evaluation[].observed_number` and `deviations[].actual_number`, which are the term being compared and assert nothing beyond the run. No new source kind and no split in the source string: a kind that branched on role would have to be chosen by the writer, and the field already says it.

**What it may never feed, which is the other half of the question.** A number sourced `simulated:` under `independent: false` may be named **only** from those two fields, never from `values[]`, and may not be carried into a KB entry or any other card -- the fields are enumerable and carrying is already tracked by `origin` (check 12), so this is checkable rather than a promise. **It keeps its grade**, max(E4, worst input): the grade says how far it could be trusted as a claim, the field restriction says it is not being offered as one, and neither mechanism is weakened to make room for the other. A reader meeting E5 on an `observed_number` learns the true thing -- this reading is no stronger than what went into it.
...
**That paragraph is gone, and it lasted six and a half hours (2026-09-20).** It said a run's observables stay in `runs/<id>/observables.json` and cannot enter a card, and that the absence of result-card code was therefore not a defect. **It was true when written**: `57f44aa` defined `simulated:` at 15:15 and neither `common.schema.json`'s source pattern nor `SOURCE_GRADE` had it, so check 1 refused the kind its own author had just defined. `1949c0e` put it in both at 21:50 and `a8c6470` closed the rest at 21:56. Read in order, the section then said *the kind exists* → *the role splits by field* → *until the kind exists, nothing may be written*, and the last one is where a reader stops. **A paragraph that records design running ahead of the contract has to carry its own expiry condition**, because this repository's shape -- `plan.md` first, `contracts/` after -- manufactures "X does not exist yet so Y cannot" on a schedule. `manager-simulation` raised it; `simulation-3` counted its first diagnosis down from "contradictory at birth" to this.
...
**Deleting a trajectory is allowed and the trigger has to be derivable (2026-09-22).** The person asked to keep trajectories and later delete the bad ones with the reason kept -- relayed through `manager-simulation` and then **approved by the person directly on 2026-09-22**, so the policy stands and not only the analysis. **No principle blocked it**: P9 governs the record, P12's cost column had over-read P9 and is corrected above, and §4.3.2's *append-only* says how the record grows rather than that data may never leave. **Mechanically it already works**: `trajectory_meta.json` holds frame count, step count, maximum displacement and stop reason in 314 bytes, so positions can go while what the trajectory *was* stays.

**What must not move is who decides.** A seat declaring a run bad and deleting it is the most expensive version of the shape this document caught three times today -- check 6 recomputing `met`, check 72 recomputing `within_tolerance`, a false backend label caught because the numbers disagreed with it. Those left a wrong claim behind; **this one leaves nothing**, and **deletion is the single operation that destroys the evidence of its own justification.** So the justification has to be checkable *before*, from what remains: **the trigger is a criterion the plan declared before the run, or a named `falsifier` that fired** -- never an opinion formed afterwards. An opinion formed afterwards has to become a declared criterion in a new revision first, which makes the deletion a derivation rather than a verdict. `manager-simulation` argued this and it is the right argument.

**A rule that is right on the case you would test first hides its own defect (2026-09-22).** The row above told check 77 that a fired criterion reads `met: false`. **A stop criterion fires by being met**, so the rule was inverted there: the trajectory of a run that did **not** diverge could have been deleted, and one that **did** diverge could not. What makes this its own variant is that the wrong rule and the right one **agree completely on success criteria** -- the case anyone reaches for first, and the case a fixture would most naturally cover. `manager-simulation` did not find it by reasoning; it found it looking at a real result card carrying `step_displacement_diverged: met=False` and reading what that sentence actually means, which is *the run was fine*. **The seat implemented the correct rule and left the declaration wrong rather than following it**, and asked for the row to be fixed -- which is the order this document wants and the opposite of taking the spec on trust.

**The falsifier route may not be the cheap one (2026-09-22).** `manager-simulation` asked whether it is a hole that a `criterion` deletion can be recomputed -- the result card must record that criterion `met: false` -- while a `falsifier` deletion cannot be, since whether a falsifier fired is usually a fact outside the card. **Its reading of why is right and the conclusion does not follow**, and the seat wrote the reason itself in the next sentence: **the falsifier path is then weaker and whoever deletes can choose the weaker path.** Not by malice -- by friction. **When two routes reach the same irreversible act and only one is checkable, the unchecked one is the one that gets used**, and this act is the one that destroys the evidence of its own justification.

    So both routes must resolve, by different legs. A `criterion` is **recomputed** against the result card. A `falsifier` **names where its firing was recorded** -- the wall-clock measurement that fired `a_cost_reference` is in the run log, so the fact exists in the tree even though the falsifier's text is not a thing to recompute -- and the check resolves that pointer, the way legs one and two resolve `declared_in` and the `id`. Neither route is then cheaper than the other, which is the whole of the requirement.

**And the residue carries the deletion -- in the run log, which is the one file here that is typed.** `trajectory_meta.json` is named in §4's outputs and has **no schema**, so a machine-checkable fact placed there would be prose in an untyped file, which is the defect this policy was careful about everywhere else. `run_log.schema.json` exists, the log is already the append-only record of what happened to the run, and §13.1 put the watcher's output there for the same reason. So the deletion is an event in `log.json` carrying **when, why, and which declared criterion fired**; `trajectory_meta.json` keeps saying what the trajectory was, which is what it already did in 314 bytes. **A reader finding no positions looks in the same directory, not a different one** -- and looks at the file a check can read.
...
**A configuration the person declares out is not a preference, and a goal carries it (decided 2026-09-24).** `manager-microscope-20260924-1` asked it when S3.0 on `mic-20260924-001` screened in confocal, `widefield_inline` and `widefield_side` after the person had set the light path to widefield by hand, and nothing in `goal.schema.json` could say so -- so prose did not cut, and caller_ids issued for a configuration nobody would run. **The two differ in kind.** A preference ranks options that are all in scope and is never evidence for any of them. An exclusion removes an option from scope, and scope is the person's to set, as it is in a `scope_approval`. So a goal may carry `declared_exclusions`, each naming the configuration, who declared it, when, and why in words; S3.0 records such a configuration as **excluded by the person**, never as screened out on grounds, so no later reader takes the exclusion as a finding that it does not work; and no caller_id issues for it. A declared exclusion says nothing about the configuration's merits, and a later goal inherits none of it. The schema field is the manager's.
...
**`revision` means the revision of the question, and it means that on every card kind (2026-09-20).** `common_head` declares it once and every card `allOf`s it in, but only one layer is load-bearing: `plan_approval` and `result` both require `plan_revision`, and check 7 matches them. Nothing downstream cites an axis card's `revision` at all -- check 33 and check 12 use it as a bundling key and stop there. So the two uses on disk have drifted: `sim-20260917-001` numbers by question, six cards to a revision, while `mic-20260918-001` numbers per axis (5, 2, 2, 2, 2, 4, 4). **Both are quiet today because that microscope fan-out has no plan card of its own** -- the repository's two microscope plan cards are `contracts/examples/` fixtures under different qids. The day it gets one, "which revision is this question" has two answers with citations behind each, and neither can be dismissed. **So it is defined now, while only one answer has citations.** The unit is what an approval names, because an approval does not approve one axis. A per-axis edit counter is a different fact and **gets no field**: nothing reads it, §7 rule 1 drops fields with no reader, and git already holds the edit history. The microscope numbers are then right field, wrong values -- a migration owed before that fan-out has a plan card, and until then undefined rather than wrong. Raised by `manager-simulation`, which reframed it from *which meaning wins* -- unanswerable without breaking a layer -- to *should a load-bearing field and an uncited counter share a name*, which answers without breaking either.
...
**On 2026-09-20 both halves of that were demonstrated in one settings file, and the second half came out backwards.** simulation-2 wrote to `envelope/safety.json` ten times through a python heredoc past a `Write(envelope/safety.json)` and an `Edit(...)` deny, which is the sentence above confirmed rather than news. What is news is the converse: a `Write(tasks/**)` deny **refused a read** -- `sed -n` on `tasks/000-read-this-first.md`, the card that seat is ordered to read first, while `grep`, `cat` and python read the same file. **This paragraph first said `sed` is filed under writing, flatly, and that was too broad.** simulation-4 read the same file with `sed -n '142,260p'` and was not refused, which forced the run: under the same deny, `sed -n '1,1p'` passes and `sed -n '/^# 000/,/^Written/p'` is refused, and the same regex form passes on `src/operator.py`, which no pattern covers. **Both have to hold** -- the path under a deny pattern and a regex-addressed script -- so what is keyed is not the tool name alone but **how the command was written**. The reasoning that excluded this repository's settings still fails, because it excluded them on the grounds that every entry is `Write` or `Edit` and a `Write(...)` entry did refuse a read.

**Check 53 cannot see this, and the reason is worth more than the case.** It reads whether a deny entry *starts with* `Bash(` or `Read(`, so an entry spelled `Write(` passes and the check reports that 113 entries block no reading while one of them had just blocked one. This is inside its stated scope, not outside it -- its own note excuses user-level and harness refusals, and this refusal came from a repository settings file. **It audits the spelling of a rule and not its effect**, and effect is not in the tree: the only way to learn what a deny denies is to attempt the operation and be refused. **And one attempt does not settle it, because the effect depends on how the attempt is phrased.** Two seats called the same read the same name and got opposite answers, one addressing by line number and one by regex; a third observation has six `sed` calls passing when the path sat in a shell variable, the deny target among them, because there was no literal path for a matcher to see. **The guard reads the command's surface text**, which is why it is half a boundary and not one: a seat learns nothing about its own case from a colleague's success in another form, and has to probe in the form it will actually use. Narrowing the check's patterns treats the case; it does not close the class. Reported by simulation-2, which hit both directions in one day.
...
3. One session's in-progress KB edits left check 25 red for hours and **refused every other session's commits.** The way out was `--no-verify`, twice. The gate was fixed to look at the tree the commit would create (§8), but that fixed the symptom; the cause is the shared working copy.
    ...
    **And that fix created the opposite direction, which no document stated until 2026-09-20.** The gate stopped a working-copy red from refusing your commit; it equally stopped a working-copy green from letting yours through. Check 45 matches a card's empty `degraded` against `librarian_agent/queries/log.jsonl`, and on 2026-09-20 that log held **230 lines at HEAD and 548 in the working copy** -- 318 calls that exist for a bare run and not for any tree a commit would create. So **every served card of every agent was uncommittable**, passing locally and failing in the index, and the seat that hits it cannot fix it: the log is in another agent's directory and check 35 refuses. **The ordering is therefore a rule and not a courtesy: the librarian commits the query log before any seat commits a card claiming the service answered.** An append-only file that one seat owns and another's checks read **serialises their commits**, and that is a property of the artifact rather than of anyone's manners. `manager-microscope` found it when a session stalled on A4 and put its half in its own `CLAUDE.md`; this half is §6.2's.
...
**One instrument, one holder at a time, and the person hands it over (decided 2026-09-24).** By that afternoon four microscope execution seats ran concurrently on the microscope computer -- one taking the first real acquisition, the others preparing the piezo stage, the confocal laser and the optical tweezers. The files divide by card; the hardware cannot, because the devices share cameras, lasers and a sample. A PVCAM camera belongs to one process, the tweezers take a Kinetix and lock out the others both ways, a piezo move shifts a sample another seat is imaging, and a laser lights frames it was never meant to be in. **So a seat opens a device -- a Micro-Manager load, a vendor program, a serial port, a DAQ task -- only after the person has said, in that seat's own window, that the bench is its, and it says when it releases it, leaving every device it opened in a stated state.** The person is the lock, because the person is at the bench and no file here can see who is. A seat without the bench works on paper, on mock, or on a vendor simulator. A relayed hand-over is no hand-over, for the same reason a relayed seating is none (§6.2.2).

**Before a session stops, it answers for its uncommitted work -- asked, never done for it (decided 2026-09-25).** The person asked that sessions commit and push what they made and learned once their task is done, before they close. A Stop hook (`.claude/end-of-turn-commit-check.py`) blocks a stop once when the session's own boundary holds uncommitted paths and lists them; the session then commits what is finished and passes the validator, under its own identity, pushes, writes down what it learned where its instructions put it, or says in one line why not. **The hook commits nothing itself**: in one shared working copy it cannot tell a draft from a finished card, or one seat's file from another's in a tree several seats share, and an automatic `git add` is the shared-index hazard this section exists for. It never blocks twice in a row, never for an unchanged set of paths, at most once per 30 minutes per session, and any error lets the stop through. The same command runs from any seat because it finds the script through git; each agent's settings are its manager's to wire.

**There are two working copies, one per machine, and GitHub joins them (found 2026-09-25).** This section assumes one working copy, and on each machine that is still true: every session on the microscope computer shares one tree and one index, and so does every session on the person's other machine. But the two machines push to the same `origin/main` and fold each other's work in by merge -- a push from the microscope computer was refused as non-fast-forward and turned out to be 36 commits behind, including the other machine's merges of the microscope computer's own pushes. So: **fetch before pushing, merge rather than force, and treat a refused push as news from the other machine**, never as something to push through. **A seat name is unique across both machines, not per machine**: the registry is one file in one history, and check 41 identifies a session by its committer email whichever tree the commit came from. The first collision was architecture's own -- both machines' architecture sessions committed as `architecture@seat.invalid` -- and the person resolved it by giving the other machine's session `architecture-20260925-1`. And the shared-index hazards above now have a cross-machine form: a merge brings the other machine's files into this tree under every running session here, so check with an in-memory trial merge (`git merge-tree`) and a clean index before merging.
...
**A technique is not an instruction, and it travels the way permission is forbidden to travel (2026-09-20).** The three rules above stop a message from carrying authority. They do not stop a message from carrying a *method*, and a method arrives making no claim at all -- so the receiving seat checks whether it works and never checks whether it is the seat that may run it. The librarian handed `manager-microscope` a way to verify the server: run `librarian_agent/src/` directly with its log redirected. Valid from the librarian's own seat. Not from that one, where `kb/` is the only readable directory of another agent. `manager-microscope` put it best: **it examined whether the method was sound and not whether it was allowed to run it in that directory.** It did not use it, and the librarian withdrew it on both grounds.

**Nothing would have caught it.** The root `.claude/settings.json` denies `Write` and `Edit` on the four `src/` and `kb/` paths and holds **zero `Read(` and zero `Bash(` entries**, so a manager at the root running that file hits nothing; and check 35 judges commits, while reading and executing produce none. The same settings file's `$comment` says the denies do not reach `sed -i` or a heredoc and that the real gate is pre-commit -- true of writing, and silent about reading and running another agent's code, for which there is no gate at all.

**And the hole cannot be closed by a settings file, which is worth knowing before someone tries.** Two layers and both are blocked. *The root file is shared by five seats* -- architecture and the four managers all run from the repository root, settings are keyed by directory, so one file serves all five and **can only hold what is true of every one of them**; the librarian manager reading and running `librarian_agent/src/` is doing its job and `manager-microscope` doing the same is not, and no single file expresses that. *An agent's file cannot point outward* -- a path pattern resolves against the session root, so `Read(librarian_agent/**)` from `microscope_agent/` names `microscope_agent/librarian_agent/**`, which cannot exist. Outward `Read` is inert for exactly the reason outward `Write` already is. So a boundary made of `Write` and `Edit` is not an omission: **it is everything expressible there.** The one mechanism that tells two root seats apart is the commit gate -- check 41, committer identity against `seats.json` `paths` -- and it sees commits. For reading and running, there is the seat's own discipline and nothing else, and no settings file can be made to supply more. Counted by the librarian in its own `librarian_agent/.claude/settings.json` (`Write` 9, `Edit` 9, `Read` 0, `Bash` 0), which it owns, can change, and did not, because changing it would not work.

**And it moves sideways, which is why the existing watch does not fire.** Manager to manager, one tier, no down. "Authority never flows down" is a rule about a direction this failure does not use. So the rule here is two-sided and made of prose, because no check can be made of it: **when you hand a peer a method, name the seat it is valid from** -- and on receiving one, the first question is not whether it works but whether you are the seat that may run it.

**And a method you should not have used, written down neutrally, hands itself on (2026-09-20).** The same seat later recorded *"log diverted"* in a task file as a plain note of how a number was obtained. An execution seat caught it and named the cost: **a record saying a seat diverted its log reads to the next person as a precedent that passed.** The seat had assumed the line described someone else's method; it described its own, and was worse than assumed. So the rule has a third face: **record the verdict with the method, or the record is the handing-on.** A wrong method with its wrongness attached is a lesson; the same method with only its result attached is a recipe. This repository already knows the shape from the other direction -- grounds that suppress checking (§8) -- and here it is the method that gets the free pass instead of the number.
...
rebuild/
  ...
  .claude/                 settings every seat at the root shares, and
                           mcp-preflight.sh, the SessionStart hook that says out loud
                           what a session cannot find out for itself and no check can
                           see: that it will have no librarian tools (11-18), and since
                           2026-09-23 which committer identity `.git/config.worktree`
                           has pinned for everyone. **Its subject is what lives outside
                           every tree** -- the name is narrower than the job and is kept
                           because renaming a registered hook costs three edits to buy
                           a word. Reports, never blocks
  ...
  microscope_agent/                 three top-level folders + CLAUDE.md
    ...
    findings/<seat>-<YYYYMMDD>.json the day's findings a seat hands to the librarian, one file per seat per day (`contracts/schemas/findings.schema.json`, `9fc1645`): each item a fact, how it was established, its kind and a source the kind requires, an observable's value always `for_store: false` (§11-21 condition 4). The execution seat writes it. **The librarian may read this folder, and exactly the paths a committed findings item cites, at the commit the item names (`git show <commit>:<path>`), and nothing else of this tree** -- read only, the same shape as the bridge's exception to read `questions/` (§6.2 rule 3). Widened the same evening from "this folder and nothing else", which left the librarian able to read a finding and not what it stands on, so every self-report would have been entered on the seat's word -- `librarian-20260924-2` found it and held. Reading at the named commit rather than the working copy means no other seat's work in progress is visible. An item whose source is not committed is not checkable and is not entered, and a verbatim copy inside the findings file does not substitute: a copy is the seat's word again. A message leaves no trace; the committed file is the trace. Added 2026-09-24 for the harvest the person asked for before the architecture session closed
...
3. **A revision decorates three things and none of them is the card, which was written nowhere until 2026-09-22.** The filename prefix `v<N>_` (this rule), the `-r<N>` suffix on the id, and the `revision` field. Three seats spent a day on one bug for want of that sentence: one blamed `plan_card.py`, one tried keying on `path` (which cannot hold while this rule does), one tried `card_id` (which fails on the suffix), and the answer was to strip both decorations before comparing. **The three can also disagree, and today they do.** Every generator writes the `-r<N>` suffix -- both agents' goals, the microscope's plan, the simulation's axis cards -- except the **microscope's axis generator**, which sets `revision: 2` and leaves the id alone. So `axis-mic-20260920-001-widefield_inline-a1` names **two cards**, revision 1 and revision 2, and **seven pairs collide that way right now.** 

   **A card id is not unique; the pair `(id, revision)` is.** That is the invariant, and the one consumer that resolves a card by id already honours it -- check 8 compares the ledger's `card_id` **and** `revision` against the payload -- which is why this has not bitten. **Nothing enforces either half.** No check is declared for it today: making ids unique means regenerating cards already committed, and `refuse_overwrite` refuses that for the right reason. Written down, and the next generation carries the suffix.

4. **A round is `r<N>_` and a revision is `v<N>_`.** Digging folders only adds depth and stops the listing being readable at a glance. **They are different things so their prefixes differ** — until 2026-09-18 this rule said "rounds and revisions go in the filename prefix", **binding the two into one sentence**, and check 13's implementation picked the first and compared `r<N>_` against the card's `round` field. So putting revision 2 beside it as `r2_` per §4.5.5 produced `the filename says round 2 and the card says 0`, and **§4.5.5 became unfollowable.** It is not that the implementation was wrong but that **when one sentence means two things, the reader picks one** — the fifth today after `accept`/`refuse`, `unconstrained`, `derived` and `degraded`. Rounds live in the bridge's `threads/<thread>/` and revisions in `<agent>/questions/<qid>/`. They never appear in one folder, so position could have done it, but **an implicit rule bit four times today, so a visible mark is used.**
...
**It is four places and not two, and check 41 taught me the fourth on 2026-09-23.** `pixi.lock` had `ALLOWED_PATHS`, `SHARED_PATHS` and a §7 listing entry, and the commit carrying the file was still refused: *seat 'architecture' is narrowed to a list that does not cover this path.* The boundary came free from `SHARED_PATHS`; **the seat's own `paths` narrowing did not**, and §11-11 is exactly that -- boundary and narrowing are two gates and a path needs both. So the full set is `ALLOWED_PATHS` (may it exist), `SHARED_PATHS` (whose boundary), the owning seat's `paths` in `seats.json` (inside that seat's narrowing), and §7's listing (check 55 reads it). **Four places, three owners**: the two regexes are a manager's, `seats.json` and §7 are architecture's. And the `seats.json` edit **cannot ride with the file it authorises** -- check 41 reads the registry at the commit's parent, so it goes alone and first. Nothing counts these four against each other, which is the same gap this paragraph opened with, one place wider.

**And the identity the whole mechanism reads is shared mutable state that no check can see (2026-09-23).** `.git/config.worktree` holds `committer.name` and `committer.email`; those beat `user.*` and lose only to the `GIT_COMMITTER_*` environment variables. **It is one file for every session and it is in no tree**, so the commit gate -- which judges the tree a commit would create -- can never look at it, and seats writing their own identity into it are **racing, with last writer wins.** Two outcomes and the loud one is the lucky one: a seat whose boundary does not match the winner is refused by check 41 naming a seat it has never heard of, which is what happened to `manager-simulation` and to me within an hour of each other under `seat:bridge-4`; **a seat whose boundary happens to match commits silently under another seat's name**, and nothing anywhere records that it did. So §6.2.1's attribution, which §8 calls the one mechanism that tells two root seats apart, rests on a file outside everything that enforces anything. **The remedy is not a check and cannot be one.** It is reporting: the SessionStart hook in `.claude/` now prints the pinned identity and where it comes from, because that hook is the only thing here that runs before work starts and can read outside the tree -- the same role it already plays for a `settings.local.json` the gate cannot see. **And the instruction is not to write your seat into that file**: that is the same race one lap further on. Pass the environment variables on the commit, where they live for that commit only.
...
**A field the gate reads and a field that records what happened move at opposite times, and I conflated them (2026-09-22).** Sending the capability table's four changes down before the first HOOMD run, I wrote *the table must change before the run* and applied it to all four. `manager-simulation` refused one and was right. **`output_independent_of_input` is read by the gate** -- twice in `validate.py`, when the result card is written -- so it must be true before the act it gates. **`executed_by` records which backend ran** and is read by **no code at all**, which today's census counted; moving it ahead of the run would make the table say `hoomd_backend` while every run on disk is the mock's. That is the defect `simulation-2` caught on 2026-09-21, reproduced with the direction reversed -- and the field exists *because of* that defect. **A record turned into a prediction loses the only thing it was for.**

    So the rule is per field and not per table: **a field a check reads is a precondition and moves first; a field that states what happened is a record and moves after.** Nothing in the contracts marks which is which, and the two live side by side in one object. The cheap test is the census's: **count the readers.** Two readers and it gates; none and it records.
...
**A comment of mine cited a check that does something else, and the citation is why nobody looked
(2026-09-23).** Adding `scipy` I wrote into `pyproject.toml` that *check 79's asymmetry covers the
residue: an import with no declaration fails, a declaration with no import reports.* **Check 79 compares
`.gitignore` against `SKIP_DIRS`** and has nothing to do with manifests. The asymmetry was real and mine
to state; the check carrying it did not exist, and naming a number made it read as though it did. So the
residue I called covered was uncovered for a day -- exactly how long `gsd` sat imported and undeclared
while every simulation run wrote no trajectory, honestly recorded as `written: false` with a reason, so
nothing went red. **A wrong citation is worse than none**: absent, a reader asks what covers this;
present, the reader stops. Nothing checks that a prose reference to check N describes check N, which is
§8's own class -- *a check that sees whether a name resolves cannot see whether that name is right* --
met here with the name resolving and the claim false. Check 82 is what should have been named.
...
**What fired decides the kind, and that is the part I missed.** This falsifier did not fire because the
world moved the way it anticipated; it fired because it was **misspecified** -- the condition it named,
a `hoomd_backend` smoke run, is not the condition that makes its two estimates measurable, which is a
run that writes a trajectory. The world moving only exposed that. **A falsifier that fires as designed
is a result about the system; a falsifier that fires because it was wrong is a defect in the
falsifier**, and those do not belong in the same place. The second goes to `failures.jsonl` -- append-only,
carrying `qid`, already holding this question's `deviation` and `abandoned_attempt` entries -- and `v3`
is never touched. The revision comes later, with `013`, and then the corrected condition fires for real. The shape is worth noting because the same seat named it about the mock the day before -- *delete or keep*, where the third was *keep and remove the silence* -- and met it again here as *edit or leave*, where the third is *leave and revise*. **Twice in two days the right answer was outside a two-option frame the seat had already learned to distrust**, which says the frame is cheap to fall into and not that the seat is careless. **And the two have one shape, which makes the rule usable.** *Look for a third option* is advice nobody can execute. But in both cases **the two visible options were about the artefact and the missing one was about the record** -- keep or delete the mock, where the third was keep it and remove the silence; edit or leave the card, where the third was leave it and record the firing. So the executable form is: **when a two-option frame appears and both options ask what to do with the artefact, the missing one is usually what to do with the record.** It reaches a third case the same day: architecture weighing whether the `tree:` line's count was a *limit* or a *defect*, both of them verdicts **about the line**, where the third was to leave the line and make it say whether two runs are comparable -- which is how it actually ended. `manager-simulation` found the shape after being shown the second instance.
...
**The judging standard is the registry as it stood at that commit's parent.** Not the `seats.json` in the current working tree. `d6f5323` showed that on 2026-09-17 — when it was made, `contracts/capabilities/microscope.json` was that seat's path, and minutes later when the boundary was divided **the same commit turned into a FAIL.** If the retrospective sweep §8 prescribes (`--commit-range`) turns old history red every time a boundary moves, nobody runs the sweep. What check 41 asks is "**was that seat inside its boundary then**", and that is a question about the commit's moment. **The consequence for a new seat is a rule, and it lived until 2026-09-20 only inside one seat's note in `seats.json`: REGISTER FIRST, COMMIT SECOND.** Because the registry is read at the parent, a seat's registration takes effect from the **next** commit on -- the commit that adds the entry cannot itself be made under the new identity, and neither can anything before it. Those commits stay under whatever identity made them and are indistinguishable in the history from another session's, which is the loss `one_identity_per_session` names. **Registration is never retroactive**; `librarian-3`'s note claimed it was and had to be corrected. **The sharpest form of it is `bridge-4`'s, measured rather than recalled (2026-09-22): being in the history and being in the history *at that commit's parent* are different things, and only the second counts.** It ran `--commit-range` over its own two commits with the registering commit already an ancestor of HEAD, and both still read PENDING. That is the sentence to give anyone who assumes a row landing now repairs what came before, because the registry visibly contains the seat while the check visibly does not see it. `architecture-2` came one commit short of the rule rather than eight, and this sentence is here because tidying that seat's note would otherwise have taken the rule with it. **And the rule is being lost at a rate, because the sequence has three hops and seats commit at the first (2026-09-20).** The person seats a session; the session tells architecture; architecture registers; only then may that seat commit. Four commits went out today under names no row covered -- `aeaa27b` under `microscope-5`, two under `simulation-5`, `23c2838` under `simulation-6`, the last of them **the first result card to reach an agent tree**. All four are permanently unattributed and re-committing repairs none. The rule is not wrong and it is not being disobeyed; **it is losing a race it was never given a way to win**, since the seat has work in hand at hop one and the registration cannot arrive before hop three. **A list that classifies paths for history has to keep every name history ever had, and a list that governs what may be written now has to drop them (2026-09-20).** `manager-bridge` found this removing `plan_ko.md` from `contracts/validate.py`: out of `ALLOWED_PATHS` it must go, since the file is gone and leaving it silently permits a resurrection, but out of `SHARED_PATHS` it must not -- checks 35 and 41 walk history, three commits touched that path, and removing it reddened all three. **Two lists answering different questions under one name.** The registry escapes the same trap by a different route: `seats.json` is read *at the commit's parent*, so taking `plan_ko.md` out of two seats' `paths` an hour earlier moved nothing historical. One mechanism versions itself and the other does not, and a seat editing either should know which it has.
...
53. An agent setting's `deny` does not block reading — it refuses entries beginning with `Bash(` or `Read(` (§6.2). `contracts/` is the place everyone reads and nobody writes, so **a rule blocking reads is not broader than the intent but contrary to it**, and a blocked seat **guesses** instead of reading. **What it cannot see is written with it, and on 2026-09-20 that note named the wrong blind spot.** It said the only refusals out of reach are user-level or harness ones. **A repository settings file refused a read that day**: `Write(tasks/**)` in an agent's own `settings.json` stopped `sed -n '/^# 000/,/^Written/p'` on the card that seat is ordered to read first, because the harness files `sed` under writing -- and this check passed it, since the entry is spelled `Write(`. That is inside the scope the note claimed to have. **The real limit is that this audits how a rule is spelled and not what it does**, and effect is not in the tree: the only way to learn what a `deny` denies is to attempt the operation and be refused. So its own line -- *N deny entries block no reading* -- claims more than it can support and should say that no entry **spelled** `Bash(` or `Read(` blocks one. Narrowing the patterns treats the case and does not close the class (§6.2). Unwritten, a pass reads as a guarantee.
...
86. **An operation plan holds to §11-21's conditions for a move that verifies.** A plan card carrying `operation` (`verifies`, `falsified_by`, `device`, `moves[]`) lifts the full pipeline's required set and refuses the S3 to S5 fields; without it the plan schema is unchanged, and its approval is the ordinary plan approval. The check holds what a schema cannot: every commanded position inside the envelope's limits for its axis, a missing limit refusing; each sine's centre plus or minus its amplitude inside them; each step carrying a role (`verify_mapping`, `direction_finding`, `approach`, `return`); and the move before a sine on its axis landing on that sine's start, because an axis at rest at 0 um with a sine centred at 300 um would otherwise open with an unapproved 300 um jump -- which is how microscope-20260924-2's read-only run found the rule. It **reports** each sine's peak speed and does not judge it, since no limit bounds a piezo's speed; whether one should is the person's. Implemented by `manager-microscope-20260924-1` at `cd2d5b7` with ten fixture groups watched failing; declared by architecture. **Declared 2026-09-25, for the tweezers form (§11-21)**: a plan carrying `trap_steps` and no goal is held to the same things at commit time. That means every position inside the person's `tweezers_trap_position_<objective>_{min,max}` for the block's `objective`, every strength inside `tweezers_trap_strength_{min,max}`, and each move within `tweezers_trap_step_max` of that trap's previous absolute position in the plan, with a first move of an unplaced trap refusing. Any missing limit refuses. The plan must also hold at least one `hold_for_person`, since on a blind channel that is the only read-back there is. The run-time gate at `759e9f0` checks the first three already. This moves the refusal to the commit, where a plan outside the person's limits is caught before anyone approves it. The implementation and its fixtures are the manager's, and until they land the run-time gate is the only check. **Implemented at `261aa36`**, which made a limit missing from the tree's envelope PENDING and asked to be corrected. **It is corrected: a missing limit FAILS**, as it already does in the piezo form. There are three reasons. First, the declaration said a missing limit refuses. Second, two forms of one check that disagree on the same situation are the drift this record keeps paying for. Third, a limit nobody has written is not a later milestone's artifact, which is what PENDING means. A plan whose bound does not exist in the tree cannot be checked, and approving it would approve an unbounded move. What it costs is an ordering, **the person's limits land before a plan that cites them**, and that ordering is the right one.

85. **A run with no plan is a preparatory run and holds to §11-21's four conditions.** `plan_id` and `revision` are null together; with them null, `no_plan_because` and `approved_commands` (`{path, sha256}` of the command list the person approved before light) are required, and with a `plan_id` both are refused. Nothing is dispatched to the motion set, and no result card or KB entry cites the run as its own. **The motion set is the devices that change where the objective, the sample or a trapped object sits** -- the Z drive, the nosepiece, the motorised stage, PFS, the piezo stage and the tweezers, under their registry ids and their Micro-Manager labels. Selectors -- mirrors, filters, ports, the condenser -- are outside it, so the first run's load moving the Lapp mirror by its Startup preset, which the person accepted, passes. A card may be stricter than this check, and card 033 was. **A tweezers command with the beam confirmed off is not motion, on two statements** (ruled 2026-09-24): `optical_tweezers` is in the set by channel because its laser cannot be read back and is assumed on (§4.6.6 rule 5); the person is among the readable sources that can show a beam blocked. So a command to it falls outside the set only when the log carries, **before the first such command**, the person's statement that the trapping laser is off at its hand control **and** that no sample is mounted -- each alone leaves nothing trappable, and two independent ones are asked because the laser's state is exactly what software cannot see. Without both, a trap command is motion and takes a plan. Raised after microscope-20260924-4, holding the bench for a laser-off software check, moved traps at the person's direction with no plan and reported it itself. Implemented by `manager-microscope-20260924-1` at `9609a57` with its fixtures watched failing; declared by architecture.

84. **The seat registry is internally sound.** Every `committer_email` is unique and equals its `seat` plus `@seat.invalid`, because check 41 identifies a seat by its email and a duplicate passed the gate while silently re-routing another seat's commits. Every manager-named entry carries `contracts/seats.json` in `excludes`, because one without it passed check 41 on both that file and this one -- a design seat able to rewrite the list of what it may write. Every entry added after the dated form's epoch is dated, `<role>-<YYYYMMDD>-<n>` parsed from the right so a role may contain hyphens, with a date that exists on the calendar; the entries present at the epoch keep their older names. **The epoch lives in the registry**, as `dated_form_from`, beside `enforced_from`, and the check falls back to a constant only while the key is absent. Finding it took a measurement: at the commit that recorded the form, the registry already held 36 entries including two dated ones, so the 34 it grandfathers are the registry *just before the first dated entry* -- that entry's parent, 34 entries, none dated. It **reports rather than fails** a design-owning entry with neither `paths` nor `excludes` other than `human`, which today is only `design`, the identity from before the seats were divided. Three fixtures, one for each failure that was measured before the check existed.

83. **A run that wrote a trajectory still has it, or the absence is said.** `trajectory_meta.json` now has a schema, applied through an `artifact: trajectory_meta` discriminator so that the 44 files written before it are not suddenly judged; with the discriminator added, all 44 pass -- 11 old GSD trajectories, 1 that could not be written, 32 from before trajectories were written at all. For a text trajectory the schema requires the SHA-256 of the file's bytes, a hash over the positions as an array, and a digit count that follows the source dtype -- 9 or 17, and 12 is refused -- tested seven ways. The check **reports and does not fail** a trajectory that was written, is gone, and has no pipeline deletion, as *deleted outside the pipeline*, naming the rerun that regenerates it and the hash that verifies it, because the person deleting is intended and the absence going unseen is not. **It prints its denominator**: on the real tree, 11 written, 11 present, 0 deleted by the pipeline, 0 outside it.

82. **Every third-party module an agent imports is declared in `pyproject.toml`**, across the `[project]` list and the pixi tables. **Asymmetric, and the asymmetry is the design**: an import with no declaration **fails**, because that code cannot run in that environment on another machine; a declaration with no import **reports**, because the person may provision a session ahead of its work -- which is `scipy` today, and the line says exactly that. **Read by AST, not by regex**: a regex pass returned five phantoms (`an`, `the`, `it`, `one`, `fields`) by reading *from ...* in docstring prose as an import, and counted sibling modules as third-party. **It prints its denominator** -- *7 modules across 59 agent source files, of 8 declarations* -- which is the rule recorded the same day for how a count reports. It is the check a comment of mine claimed already existed: I wrote into `pyproject.toml` that check 79's asymmetry covered this residue, and 79 compares `.gitignore` with `SKIP_DIRS`; `gsd` then sat imported and undeclared for a day while every simulation run wrote no trajectory.

81. **Every relative import in an agent's source resolves inside the tree the commit would create.** On 2026-09-23 `e8b8382` made five axis modules import a module in nobody's commit and passed the gate, because **the gate runs the validator and the validator may not import agent code** (check 16), so a commit breaking a whole fan-out goes green. Read **statically** -- parsed, never executed -- which is how it clears the reachability question, using check 16's own extraction. 5 detections against the broken commit, 0 against the tree after, over 47 relative imports. **Two limits are part of the declaration and not of a message.** It catches the *symptom* of a sweep-in rather than the sweep-in: three happened that day and one broke an import. And it would refuse the whole of the offending commit, so **a seat that named only its own paths is blocked by another's hunk** -- correct to block, and not what this check claims to be about.
...
**Forms 4 and 5 landed on 2026-09-23 and each taught something the first three had not.** Form 4 --
subscript and `.get()` keying, which carries no `Compare` node -- **found a second site nobody had
counted**, `plan_card.py:105` keying `intermediate_magnification` the same way. Form 5 nearly could not
be written honestly: there are 23 `startswith`/`endswith` calls in these trees and **21 of them parse
source prefixes and file extensions**, so a check firing on all 23 would cry on 21 correct lines, and
**a check that cries on correct lines is skipped** -- the same rule as §2.1's guard that is harder to
turn than to skip, applied by a seat to its own check. `manager-simulation` split on the **receiver's
binding** instead: a name bound by iterating `element_ids()` and its siblings. **That accessor set is
itself recognised by spelling, which is this check's own instance of the defect it detects**, and the
seat wrote that into both the docstring and the output line rather than leaving a reader to find it;
the line prints `counted / 23` so the ratio is visible. The fixture that matters is the one removing the
receiver filter: the count goes 5 to 6 and a planted quiet `startswith("kb:")` fires, which shows **the
filter is working rather than that no such call happens to exist.**

**Two further forms were named in the declaration before implementation, which is the order that matters.** `state.get("nosepiece")` has no `Compare` node at all, so the AST walk cannot see it; `microscope-1` found it by reading, after the three forms were declared. `manager-microscope` **declined to add it on its own**, and was right: adding a form the declaration does not carry makes **the guard's scope set by something other than a declaration**, which is the defect §2.1 rule 2 exists to remove, committed by the seat removing it. So it is declared here first and implemented after -- together with `startswith` and `endswith`, which `retract_elements()` uses and which the original three do not reach.
...
77. **A trajectory deletion names what triggered it and that trigger is checked.** `triggered_by.declared_in` resolves to a plan card in the tree, its `id` is declared there, `what` is actually gone, and for `kind: criterion` the run's result card shows that criterion **fired** -- `met: false` for a **success** criterion and `met: true` for a **stop** one. This declaration said `met: false` for both until 2026-09-22 and was **exactly inverted on stop criteria**, which would have permitted deleting a converged run's trajectory and refused a diverged one. The wrong rule and the right one **agree completely on success criteria**, the case anyone writes a fixture for first. `manager-simulation` implemented it correctly from the reasoning and asked for the row to be fixed rather than following it. For `kind: falsifier`, the `recorded_in` path resolves in the tree.

76. **A settings file this repository cannot read is reported beside the ones it can.** Where a tracked `settings.json` sits, a `settings.local.json` beside it is named, with the **names** of its MCP and permission keys and never their values. **Advisory by construction and its own line says so**: the file is in no tree, the gate's exported tree cannot hold it, and only a working-copy run sees it -- the relative of `--commit-range` freezing the data and not the checker. It emits PASS and N/A and never FAIL, because the file may legitimately be there and a gate refusing over what it cannot see gets worked around. Two fixtures, each killed only by the mutation aimed at it.
75. **A bench-source entry says what ends it** -- an event, or that no event does and why, or that it is not known. Advisory while the backlog stands and enforcing at zero, the shape check 29 took for `task`/`occasion`. The prefix decides which entries are in scope: a claim about this bench ages and a claim about a product does not.
74. **A card standing on a round is compared against it.** `from_round` resolves to a thread and round that exist, that round was delivered to **this** agent, and its source card is still at the revision the ledger pinned. The third **reports rather than refuses**: a round going stale under a goal is not the goal's fault, and failing the seat that cannot act is what check 50 forbids. Nothing read `from_round` until this -- **the field the bridge's loop closes on was checked by nothing**, while `payload_hash`, `canon_sha` and the ledger's own pin were each checked twice.
65. **A check that reads git history has a fixture that builds one** (§11-7). The four are 26, 35, 41 and 46; 35 and 41 are covered and 26 and 46 are not, and the file says so rather than implying coverage it does not have. Repositories are built and not stored.
67. **A KB entry uses declared units everywhere a quantity appears in it** -- `numbers[]` and `validity` conditions both, the way check 2 requires of cards. Check 2 reads cards only, so the store's numbers were clean because a person cleaned them and nothing refused the regression.
68. **A gap names the quantity it wanted, never the subject or the locus** (`quantities.json` rule 1). A name that asserts its own subject is a subject nothing can refuse. It buys naming hygiene and not safety: the 2026-09-20 case returned `absent` and abstained, where obeying the rule would have returned the wrong material's index and computed with it.
69. **A `kb:` source in an entry may not resolve to the entry it lives in.** Check 54 resolves a *card's* basis and never looked at an entry citing itself, so such a grade rests on nothing and nothing says so.
70. **One `kb_version` never answered the same question two ways** (§4.3.1 rule 2). `query_log.verify()` over the whole log, with the one documented pair of 2026-09-19 excluded **by name and not by watermark** -- a watermark forgives every violation before it, naming one forgives one.
71. **Every check in §8's table is held by a seat that holds `contracts/`.** An execution seat cannot write `validate.py`, so assigning it a check is not a backlog but an impossibility. Two of mine were, on the day they were written.
...
| **Finding** | when a seat notices a defect outside its own work -- in a contract, another layer, a rule's scope | what was noticed, where, and **whether it was acted on or handed up**. Added 2026-09-20: a finding is none of the six above, so **a discovery a seat cannot act on survives nowhere on disk** -- the same sentence as the abandoned attempt, one row up. The librarian's three structural findings lived only in messages, and the one that reached architecture did so because that seat pushed it twice |

**Ruled 2026-09-20: the seventh kind and no new file.** The librarian proposed instead an append-only upward file owned by the execution seat, on the ground that `failures.jsonl` is *what went wrong* and not *what was found*. **This section is titled "Recording failures and successes" and its table already holds a `Success` row**, so the premise is contradicted by the artifact it describes: the file is the seat's append-only upward record of what happened, and has never been failure-only. A second file with the same owner, the same direction and the same semantics is **the same mechanism at a different address** -- which is the defect the librarian itself diagnosed an hour earlier about the source-grade table living in two copies. And a new artifact needs a reader; managers already read this one.

**What survives the ruling is the sharper half, and it is not a stopgap.** The librarian first said no kind fits and then narrowed it, and the narrowed statement is the real one: **a finding made inside the queue can be recorded and one made outside it cannot**, because `task` has nothing to name. Check 29 tests that `task` is present and not that it resolves -- 12 of 22 values resolve to no task file and two are whole sentences. **So `occasion` is the right half rather than a stopgap**, and the rule is: `task` resolves to a real task file, **or** `occasion` says what the work arose from. This tightens check 29 rather than adding a number -- **and it is stated here before check 29 enforces it, which this document has been told today to say out loud.** The check counts and does not fail: 20 records across three trees name a `task` that is no task file, and **no one seat can clear them** (13 simulation, 4 bridge, 3 librarian, counted by the librarian). The advisory becomes a failure **on the day the last tree migrates its own rows to `occasion`** -- that is the expiry condition, and until then the rule and the check deliberately say different things. Nothing is blocked meanwhile: the 20 are visible at every gate, which they were not yesterday. **Most of what this repository gained on 2026-09-20 came from outside the queue** -- the queue distributes what is already known, and a discovery is by definition not in it.
...
**Check 68 catches the naming defect and not the hazard, and the difference is the whole lesson (2026-09-20).** An A6 card asked `kb_gaps` for `immersion_refractive_index`, which is `quantities.json` rule 1's exact recurrence path -- the subject smuggled into the name, where nothing refuses it. The store said `absent` and the axis abstained. **Had the card obeyed the rule it would have been worse**: `refractive_index` returns eight polystyrene entries at E3, which is the bead material and not the immersion medium, and the axis computes an `axial_range` from the wrong substance with nothing going red. So a rule violation produced the safer outcome, and the check now declared for it **buys naming hygiene and not safety**. What the hazard needs is the subject actually being matched, which is check 44's territory and is why rule 1 exists at all; 68 is worth having because a name nothing can refuse is a permanent hole, not because it closes this. Found by the librarian in `manager-microscope`'s own card, the fourth time today a seat's error was caught by someone else counting.

**Stopping a practice leaves nothing behind, so nobody sees it stop (2026-09-20).** Microscope cards 000 and 004-011 each named the seat they were for. From 012 the habit lapsed, silently, and 016 went to two of the four live execution seats: both wrote the same goal path, the later overwrote the earlier, and **the earlier committed the overwrite as its own work** -- `git commit -- <path>` takes the worktree state and refused nothing. **The sharpest part is in the reporting seat's own account**: it had written to one seat that `nosepiece_position` was already 5 when that seat's card said 1, because it read another seat's file at the same path and took it for the recipient's. **The collision was inside the message it sent and it did not see it.** A missing field fails a check; a missing *habit* has no artifact to be missing from, which is why this one ran four cards before it cost anything. The fix was not more care: seats went into the cards, read off the commit history, and the standing order now says **a chat message is not an assignment -- the card is, and if the card does not say, the assignment does not exist yet.**

**Design running ahead of the contract manufactures paragraphs that expire, and they do not know it.** §5.3 carried one for six and a half hours saying a run's observables could not enter a card, true when written and false by evening, sitting at the end of the section where a reader stops. The shape is structural here -- `plan.md` first, `contracts/` after -- so **a paragraph recording that gap has to carry its own expiry condition**, not because anyone was careless but because this order guarantees more of them.

**Both of the day's families are one sentence: a rule is written as covering more than it actually reaches.** The librarian put it that way and it is better than either half. **Its own three miss a layer** -- `conflict_with` and `refuted_by` live on an entry and the projection does not carry them; `kind`, `formula`, `symbol`, `unit` and `valid_until` likewise; and check 43 reads `source` at the entry while it also exists on `numbers[]`. The first two lose information about an answer and **the third loses the answer's authority**, deriving a grade from a layer nothing looked into, which is why it had to be fixed first -- and that ordering is what makes the three a shape rather than a list. **The other three miss a side**, below. Each of the six was separately well recorded; **what existed nowhere was the claim that they are one**, so disk held six fixed bugs and no recurring shape. **The remedy is not a seventh check but a rule saying what it reaches** -- a check can compare two things that exist and cannot notice a case a rule never contemplated.

**The class is that a contract says something and there is nowhere to put it, and "the microscope side" is only why it clusters (widened 2026-09-21).** This entry first named the side and that was a **subset that cannot see its own counter-example**: `manager-simulation`, writing check 72, required exact reproduction of a planned value and wrongly failed a *microscope* fixture where the camera had quantised 7 ms to 8 ms -- the same defect pointing the other way. `manager-microscope` then counted four with no side at all: `run_log.verification`, `kb_gap.at`, `plan.schema.preconditions`, and the camera mode. The three below are the same shape read narrowly -- §5.3 defined `simulated:` and the pattern would not take it, §4.6.9 offered three means and the simulation could use none, rule 1 forbade a locus in a name with no field to move it to. **Side remains true and is the second fact**: the microscope was built first, so the empty slots appear on the other side. Written as a side, the class would read as *be careful around microscope rules* and would have missed both of today's counter-examples.

**Three times on 2026-09-20 a rule written for the microscope hit the simulation side unchanged, and fixing three instances is not the same as recording the pattern.** `simulated:` as a source kind -- §5.3 was written with `measured:` in mind and had no way for a run to state what it read. `revision` -- one name for an approval's unit and for a per-axis counter, because only one side had approvals. `time_base` -- three means of alignment, all of them naming hardware or the OS, on a side with no clock. Each was found the same way: **a seat trying to write the first card of its kind**, never by reading. The pattern is not carelessness but order -- the microscope side was built first and its vocabulary became the general vocabulary, so **whatever the simulation side does that has no microscope analogue arrives as a contract that refuses it.** `manager-simulation` raised the pattern rather than only the third instance, which is what makes it an entry here. §8 already held one of these from when check 57 was reassigned; that was the first and nobody named the class.

**And its neighbour, which is worse: knowledge that lives in prose because no field exists for it.** `cameras_both_kinetix22` already carried *Falsified by either camera being replaced* -- in `validity_conditions`, as a sentence, where nothing can read it. The fact was known, written, committed and unreachable, and the policy question that surfaced it was asked as though nobody knew the answer. A filled field hides a defect behind presence; **an absent field hides a fact behind prose**, and the second is harder because the writer did everything right.

**The shortest form of the shape this section keeps finding, in the librarian's words: a field that is filled is a field no check looks inside.** A fifteen-year date in `valid_until` while `event` was empty; `task` present and resolving to no task file; `plan_ko.md` in a seat's `paths` after the file was gone; `met` carried beside a number that contradicted it. **Presence is what schemas test and content is what nobody does**, so the defects that survive longest are the ones wearing a populated field.

**A count that mixes fixtures with real work reads as progress, and three of this run's headline numbers do (2026-09-20).** `check 33` says 19 axis cards: 13 are in agent trees and **6 are `contracts/examples/`**. `check 56` says 3 plans: **2 are fixtures**, both microscope, and the one real plan card in the repository is simulation's -- the microscope tree has none. `contracts/examples/result.json` exists while no agent tree holds a result card at all. Architecture answered the person with 19 and 3 an hour before this was caught, having read them off a run exactly as this document instructs, **because the instruction to read counts off a run says nothing about what the run is counting**. The fix is not fewer fixtures -- they are the test suite and check 13 counts them deliberately -- but a line that says which. Caught by `simulation-4`, counting its own tree against the run; a seat inside a tree can decompose a total that the seat reading the total cannot.
...
**The validator reads artifacts, and permission is not an artifact -- three times on 2026-09-20 alone.** A session's real MCP permissions sat in an untracked, globally gitignored file no check and no seat could reach (11-18). A read across a boundary leaves nothing for any check to find (below). A deny rule's actual effect is observable only by attempting the operation and being refused, so check 53 audits how the rule is spelled (6.2). These look like three defects and are one shape: **the gate judges the tree a commit would create, and what governs a session is partly not in any tree.** None of the three is fixed by a better check, because in each the thing to check is not written down anywhere. What they argue for is the opposite -- rules in this area carry a test the seat applies to itself, and the seat reports. Which puts the weight on confession, and the entry below on what that costs.

**No check can see a read, so the read half of the boundary is enforced by confession alone.** Checks 35 and 41 read commits, and a commit is a write; a session that opens another agent's file leaves nothing anywhere. Both crossings this repository knows about surfaced because the seat said so -- the bridge seat on 2026-09-19, the architecture seat on 2026-09-20 -- and neither would have surfaced otherwise. That is not an argument for a check, because there is no artifact for one to read; it is the reason the rule carries its own test ("was that conclusion reachable inside the boundary") rather than a threshold. A rule whose only enforcement is self-report needs to be **easy to apply to yourself**, and the 2026-09-20 case shows it is not yet: that seat quoted the test in its own prose the same hour it failed it.
...
| **M1** | **The microscope agent.** S2 → S3.0 → per-axis analysis → S4 → S5 → approval → execution. `envelope/safety.json` (**a person writes it**, §10.3 rule 4), the link between the optical-path table and `capabilities/` (check 38), the approval-gate hook | **Both of the blockers this column named are now closed.** S3.0 screening was blocked on §11-1, settled 2026-09-19; execution was blocked on `envelope/safety.json`, which the person wrote on 2026-09-20 (`c1404bf`, raised at `ab33089`). It was never blocked on the librarian, and the pass with the librarian on is the one condition already met -- check 45's 7 are this agent's. What remains is execution and a result card | A Tier 2 call with no approval is **actually blocked.** One pipeline pass on the mock backend, then one real measurement with its deviation record. **And one pass with the librarian on** — see below |
| **M2** | **The simulation agent.** The same five-stage pipeline, axes A1–A5 + A7 (applying only in a driven configuration and abstaining at equilibrium; no A6 — §4.5.3), the HOOMD backend | Nothing by design as of 2026-09-19, when §11-1 settled and `produces` stopped being blocked. What remains is execution | One goal → a plan that passes validation → a smoke run → convergence evidence. **One pass with the librarian on** as well. **Convergence evidence does not follow from running**: the criterion is declared in the plan before the run and the operator only evaluates it, so a plan that declares none cannot produce the evidence however many runs it has. `sim-20260917-001` declares two stop criteria, duration reached and divergence, and no equilibration or steady-state criterion at all; two smoke runs completed on duration. **The person settled it differently on 2026-09-22: a result that comes of running long enough is enough, and the person analyses it and calls for more running if it falls short.** That is not a repeal of declare-before-the-run -- it moves the verdict rather than the rule. What the plan declares stays what it always was, a planned duration and what will be looked at, because *longer* needs something to be longer than; what it does not declare is a machine-evaluable convergence threshold, because there is not one to declare. **The convergence verdict is then a decision and carries no grade**, the same as a target or a safety ceiling (§5.3.1), and **each call for more running is a revision**, which this repository already has machinery for. The discipline the rule protects is intact: it exists so the *operator* cannot choose a criterion after the fact to flatter its own run, and a person outside the system judging afterwards is §4.5.1 (c), not that failure. So M2's third stage is met by the person's recorded verdict together with the runs it rests on. **And the verdict's home is ruled 2026-09-22: a card the person writes into `<agent>/approvals/`.** `simulation-2` read `result.schema.json` field by field and found nowhere for it -- `criteria_evaluation` is the machine evaluating what the plan declared, `approval_id` points at cards that approve **before** the run and pin the plan hash, and nothing else is a judgement. **The most plausible place is the wrong one**: `outcome` is a *derived* field, so putting a decision there makes a derivation carry one, which is the collapse this ruling exists to prevent -- and with no threshold declared there is nothing left to derive `NOT_CONVERGED` from, so that enum value becomes reachable only for plans that do declare one.

    **`approvals/` satisfies three of the four constraints without inventing anything**, which is why it is the home rather than a new folder: every agent's `.claude/settings.json` already denies `Write` and `Edit` on it, so **the person writes it and no seat can**, and that guard is in place today. What the card adds is the rest: **it names a `run_id` and not a plan**, because *run longer* makes each call a revision and a verdict on run N must not read as a verdict on the plan; **it carries its own id** so the next revision can cite it; and **it carries no grade**, being a decision like a target or a ceiling (§5.3.1).

    **One constraint more than the four asked for: it says what was looked at.** A verdict recording only *enough* leaves the next reader unable to tell what the person examined, which is the `kb_gaps.searched` discipline one level up -- looked-at-and-judged and nobody-looked are not the same verdict. The schema is `contracts/` and so a manager's; this is the shape it lands against. **And *long enough* got a number the same day: a run defaults to 10 τ_d with a timestep of 1e-3 τ_d, where τ_d is `kb:tau_d`, d²/D, the time a sphere needs to diffuse its own diameter.** The person set it after confirming there was no default to override -- `plan.schema.json` declares none. **Which τ had to be asked rather than assumed**: the conventional Brownian time is as often a²/D, the two are a factor of four apart, and this repository was bitten by exactly that pair on 2026-09-17 when a card minted `tau_a = radius²/D` at E5 while the store already held `tau_d` at E4. The person chose the registered one, so a plan **cites `kb:tau_d`** and does not compute it -- §11-1's ordering did not even arise. **The multiple is a decision and carries no grade** (§5.3.1, like a target or a ceiling); **the seconds it becomes are a computation and inherit τ_d's grade**, so the two sit in different places on the card and the run's wall duration is never better than E4. **And the place for the multiple already exists: inside the `formula` (2026-09-22).** `simulation-2` went looking for a slot and found none -- `$defs/target`'s `kind` enum does not cover a duration multiple, `$defs/number` requires a `source` and a `grade` and is closed -- and then found that no slot is wanted, because `10*tau_d` is **a coefficient in a declared derivation**, which is neither a graded number nor prose. **τ_d's own formula already carries an ungraded 2** (`bead_diameter**2/diffusivity`) and nobody has ever graded that exponent. So the number goes in `numbers[]` at E4 and the 10 goes in the formula beside it, where **check 17 recomputes it.** Putting it in `numbers[]` would make it `assumed:` E5, which is the collapse this ruling exists to prevent; putting it in a `note` would make it prose nothing reads.

    **Which card holds the formula is settled by the checks and not by taste: the synthesis card, with the plan carrying it by `origin`.** Check 17 reads `if n.get("origin"): continue -- verified at its source by check 12`, so a `formula` written on a plan number that also has an `origin` **is never recomputed** -- a filled field no check looks inside, which is §8.2's shortest line. One copy on the synthesis where the derivation happens, check 17 recomputing it there, check 12 verifying the carry: 33 computed values and 146 carried ones already run that way |
| **M4** | **One bridge round trip.** Hashes, unit and dimensionless consistency, producibility | **Both halves are closed as of 2026-09-21.** §11-1 closed 2026-09-19; the `capabilities/` half closed too -- both tables carry `produces` and both name `tracer_diffusivity`, so producibility derives. What remains is one side's card: simulation has a result in its tree and the microscope has no plan, synthesis or result card in its. **This row said the capabilities half was open for a day after it was not**, the third §9 row to do that | One side's result → the other side's plan → the results compared as the same observable (S4) |
...
**The real critical path was not a milestone but one decision, and it was decided.** Reading the "what blocks it" column above, §11-1 (the observable vocabulary) blocked three of the four -- not something to build but something to settle, and only a person could answer it. That was concurrent construction's first dividend: with an order in place the fact would have hidden behind "it is not that turn yet". **The person settled it on 2026-09-19 -- no list is made, the vocabulary stays addable at any time (§11-1, 180 lines below this one).** So the `produces` declaration is not blocked, M2 and M4 are not blocked on it, and **three of the four rows above went on naming it as their blocker for a day**, this paragraph loudest of all. A table that says what to work on next is the worst place for a closed question to keep standing, because that is the table a person reads precisely when deciding where to put the day. What blocks those milestones now is **execution, not a decision**: cards, runs, and evidence, which the seat that owns each tree can count and this one cannot.

### 9.1 What concurrent construction inverts: which path goes unwalked

**The condition is read over the cards that asked, settled 2026-09-22.** Read per-card across every axis it is **constitutively unsatisfiable**: an axis with nothing to ask cannot fill `kb_refs` or `kb_gaps` however well the service is running, and three of the simulation's six are that by design -- A2 statistics, A5 budget, A7 the driven abstraction. Read over the axes that query, it is met, and **the count is the same either way: simulation is 3 of 3.** This is the exact inverse of a defect `simulation_agent/CLAUDE.md` already records, where a condition demanded what happens anyway and so verified nothing; here one demanded what cannot happen. **A completion condition has to be satisfiable by the thing it describes**, and neither failure shows until somebody tries to satisfy it.
...
**2026-09-24, the person, in the architecture session: `C:\agentic_microscope` may be referred to for the upgrade.** Said on the microscope computer, where that repository sits as a local folder, while the first real acquisition was being prepared, and after the person chose its `config/micromanager/single_cam_red_noDMD_nocom10.cfg` over the lab's own file for that run. **Every row above, as it applies to `agentic-microscope`, is open from this date**, including the two still waiting on a condition. **What it does not change**: looking is not taking, so §10.2.1's ruling stands for every item that crosses, and the person's own condition from 2026-09-17 -- nothing transplanted as-is, anything over-claimed downgraded or dropped -- was not lifted, and nothing said today reads as lifting it. §10.3 stands whole, and a safety limit still never crosses. The other three repositories stay closed; this sentence names one folder.

**2026-09-24, later: `librarian-agent` opened for ONE search, by the person.** Decided in `manager-microscope-20260924-1`'s window when the librarian found no power calibration for the confocal or illumination lines anywhere open to it, and **confirmed by the person directly in the architecture session** before this was written, because a decision to open a closed repository is not recorded on a relay. Scope: one search, at `C:\librarian-agent` on the microscope computer, for the confocal/illumination power calibration and nothing else; the search reads matched paths and match counts, and what it finds goes to the librarian to rule item by item under §10.2.1 -- nothing copied as-is, and no safety limit crosses (a calibration converts a setting to a power; the limit on that power is still the person's to write). **It is not an opening of the repository**: every other use of it stays forbidden, filenames included, and `Brownian-Dynamics-Agent` and `sim-exp-bridge` stay closed. The person also pointed to a worksheet outside every repository, `illumination-power-worksheet.xlsx` (2026-09-09), which is the librarian's to assess as a source on its own terms.
...
**There is no fourth word for "used where it sits"** (decided 2026-09-24). An artifact loaded in place -- a Micro-Manager configuration read from `C:\agentic_microscope` by path, never copied into this tree -- still crosses: its labels choose which body a name opens, its presets write to devices on load, and **it carries figures invisibly.** Measured on the day: the configuration chosen for the first real acquisition holds a pixel size for each of the six objectives, the 20x at `0.32373` um -- five figures nobody here measured, the shape `20.078x` already was -- and Micro-Manager stamps it into every frame's metadata. A word that names no place and no rule is exactly the hatch this subsection exists to close, so such an artifact is ruled like anything else, piece by piece: the file itself is a **transfer** whose place is the run's loaded configuration, recorded by path and sha256; its labels are **downgraded** until a frame confirms what each opens; its figures are **discarded as a source** -- a card takes a pixel size from the store, and a run log says when frame metadata carries a number that did not. A preset the person accepted is not ruled at all; it is the person's decision and recorded as one.
...
2. **The E5 ceiling**: how many estimates are allowed in one plan? Zero and nothing can be done; unlimited and the grade system is meaningless. **It is settled only after both conditions hold**: ① gap detection is on (the librarian service), and ② plans made in that state have accumulated into a sample. Concurrent construction advances neither — when the librarian comes on is not fixed by an order, so ①'s moment is itself open (§9). Until then check 3 is `UNDECIDED`.
    ...
    **And the choosing needs to name which number it is choosing, because the two differ by more than a factor of two:**
    ...
    Check 3 counts **distinct rationales** and says so in its own line. A cap on that limits **how many different things a plan is allowed to guess about**; a cap on chain length limits **how much of the plan rests on guessing at all**. The first is the one that keeps a plan honest about its own shape, and the second is what P15 already governs through precision inheritance. **Settled 2026-09-22: the cap is on distinct rationales, and it is 7.** The person chose the measure first and the number after seeing what the existing plans carry. **7 is the largest a real plan holds** -- `sim-20260917-001` sits exactly at it -- so nothing that passes today starts failing, and the next plan cannot exceed the worst one already written. The check reads `if n > cap`, so 7 passes and 8 refuses. **It does not bite yet** -- `contracts/validation_limits.json` holds `max_rationales_per_plan` and it is still `null`, that file is a manager's, and **check 3 stays `UNDECIDED` until the number is in it.** Declared here and enforced there is this document's usual split; what is unusual is only that the gap is one value wide and visible in the run's own line. What the choice buys is that the cap limits **how many different things a plan is allowed to guess about**; what it deliberately does not do is limit **how much of the plan rests on guessing**, which stays with P15's precision inheritance -- one estimate in a chain already makes the answer an order of magnitude, so the chain length of 8, 17 and 14 is governed and not ignored. **Nobody should later read this choice as chain length not mattering.** The paragraph above still holds for the number itself: a sample gives grounds for choosing and does not do the choosing.
...
23. **The first simulation-experiment comparison, and when a quantity is comparable across the two sides** (raised and partly settled 2026-09-24, for the person's plan to test an asymmetric double well with one trapped particle on 2026-09-25). **The person decided the model**: two optical traps as two Gaussian wells that add, `U(x) = -eps_1 exp(-(x-x_1)^2/2w_1^2) - eps_2 exp(-(x-x_2)^2/2w_2^2)`, each harmonic at its bottom with stiffness `k_i = eps_i/w_i^2`, the barrier coming from each well's decay beyond its beam. That is what `proto_double_well.py` integrates, and it replaces `sim-20260923-101`'s draft combination rule `min(U1, U2)`, which the card itself called a convention and not optics. **What the comparison waits on**: the model declared as a configuration in `contracts/capabilities/simulation.json`, so its runs carry the `simulated:` grade §5.3 gives a declared configuration and a prototype's numbers do not; the microscope's run as a planned measurement, not a preparatory one; and the four observables -- `well_occupancy`, `well_residence_time`, `interwell_transition_rate`, `interwell_barrier_height` -- turned comparable. **When they may be** (settled here): all four are read from a one-dimensional trajectory along the line joining the traps, so they are comparable when (1) both sides apply the same registered estimator, milestoning with the same definition of each well's minimum and of the barrier region, to that trajectory; (2) the potential's parameters are stated on both sides in SI and mapped explicitly -- each trap's stiffness calibrated on the experimental side trap by trap, the separation, each trap's width, the temperature and the viscosity or diffusivity -- because a comparison in reduced units against a measurement in micrometres compares nothing; (3) record length and sampling interval are stated on both sides, since three of the four are window-dependent by definition; and (4) each number carries its own side's grade, `simulated:` from a declared configuration and `measured:` from a planned run. The flag in `observables.json` is the manager's, per observable, once those hold; differences under a decade are ties (P15). **The one-dimensional cost is measured and avoided** (the same night): `manager-simulation-20260924-1` declared the model as `bd_overdamped_gaussian_double_well_2d` (`4ab86d9`), in two dimensions, and compared it with the 1-D prototype on the same potential (well depths 6 and 5 kT, separation three widths, 4000 walkers). The 1-D reduction overstated the transition rate by about 40% and the deeper well's occupancy by about 13%. Those are engine measurements made in scratch, carried by no card and graded nothing. Both differences are ties under P15, and the declared configuration does not make the reduction at all. HOOMD 7.2 in 2-D agreed with numpy on occupancy to within 1%; the rate differed by 7%, unexplained that night and assigned. **What stays open**: whether a stiffness ratio of 100 leaves the weak well deeper than a few kT at achievable powers is the experiment's feasibility to establish, not something either side may assume. **The simulation's map narrows it** (`sim-20260923-101` revision 3, the ask `result_run-20260924-101-v3-hold` at `735a1a0`, `simulated:` and assumed inputs, E5): a ratio of 0.5 or below leaves no barrier in the 1-10 kT window at any separation, the two stiffnesses must match to a few per cent at 1 pN/um and about forty per cent at 0.1 pN/um, and the barrier moves about half a kT per 10 nm of separation at 1 pN/um. So the operating point is soft, near-equal traps with the separation tuned by the histogram, and a ratio of 100 is not one; the bench's calibration is what tests the map. **The first run starts before the round is delivered** (the person, 2026-09-25, by direct answer in architecture's window, with the bench waiting): the ask was committed at 09:10 and the bridge had not delivered it. The person chose "Start now" over waiting, so the bridge files r1 after the run, the microscope goal card's `from_round` is linked then, and the thread says the run preceded delivery. **Who drives, and the exception to D12**: offered session 6 or architecture, the person first chose session 6 and minutes later reversed it in the same window -- "or you can operate tweez300. I can run micro-manager myself", then "i still prefer running it yourself. Try again". So **architecture operates the Tweez300 over TCP** through `microscope_agent/src/devices/python_tcp.py` unchanged (trap creation, strengths, positions; `LASER_ON` stays refused there, the dial stays the person's), and **the person runs Micro-Manager and the camera by hand**. Architecture touches no other device. What that costs, said to the person before the choice: a root session loads none of `microscope_agent/.claude/`'s guards, and check 35 refuses architecture's commits into `microscope_agent/`, so the command log is kept outside the tree and a microscope seat commits it. The hand-over is the person's statement in architecture's window, with 047's four statements asked before the first command; the person's direct instruction there stands in for an approved plan for this run and is recorded as such, not as one. The person's protocol, their words: "start with same trap strengths in Tweeze300 software for both traps. then decrease one trap's strength sequentially." **The exception ran about half an hour and closed the same morning.** The person then said "all of hardware are yours" and delegated the dial, objective, height and bounds ("You can figure out 3-6"). Architecture loaded the camera through `GuardedCore`, switched on the dia lamp at the person's "you can turn the lamp on yourself", and sent the tweezers only the readiness probe. The three 5 um beads in view were immobile and in the coverslip's focal plane, so none looked trapped above it. The person stopped architecture's bench process, and asked session 6 to "trap two particle and bring them near the center" ("two separate particle"), then withdrew it in architecture's window: "no two particles, it was my mistake". The experiment is still the one-bead double well. Asked who drives, the person answered "session 6 will do". **Architecture holds no device from then on**, and the next holder takes the bench from the person in its own window. The log is `D:\soft-matter-agents-frames\run-20260925-001\` (bench_log.jsonl, tweezers_dispatch.jsonl), for a microscope seat to commit. **One finding for whoever holds a bench next: the dia lamp read `State 0` after Micro-Manager closed and the configuration loaded**, so a brightfield frame was dark until it was switched on. The allow-list refuses `DiaLamp` by name, so turning it on went through the raw core, at the person's word, and was logged as such. (`manager-microscope` has since put `DiaLamp` State and Intensity on the allow-list, `c13c496`.) **The trap-step exemption, accepted by architecture the same day** (built by `microscope-20260924-6` at `759e9f0`, brought by `manager-microscope`). It is the second software-motion exemption after card 040's. Commands to `optical_tweezers` pass the allow-list only when derived from an approved plan's `trap_steps`, a plan shape added at `4ba2d1c` that holds absolute positions only, `read_back` none, and a `hold_for_person` step. The approval must cover this revision and hash, found in the call that grants. Each command must equal its derivation. Every position must lie inside the person's `tweezers_trap_position_<objective>_{min,max}` for the objective the person states at hand-over, which must also be the plan's. Strength stays inside `tweezers_trap_strength_{min,max}`, and each move stays within `tweezers_trap_step_max` of that trap's last absolute position. A missing limit refuses. Steps go out one at a time in the plan's order, and an unconfirmed hold or a step not accepted in full stops the rest. **Why it is accepted**: a trap command moves where the beam sits and shares out the dial's power among traps. It cannot raise the total and cannot switch emission on, because `LASER_ON` stays refused in `python_tcp.py` and the dial stays the person's hand. So its failure modes fall on the sample, a lost bead, not on people or the instrument. The bound is the person's own limits (envelope policy 8: ±40 um at 100x, ±67 at 60x, ±100 at 40x, strength 0-1, 1 um a step). Architecture re-ran the evidence rather than taking it relayed: `tests/test_trap_steps.py` 19 of 19, `tests/trap_steps_mutations.py` 11 mutations all red. **Not bounded, noted rather than blocking**: a strength change has no step limit, so one plan step can drop a trap from 1 to its floor and release the bead. That costs a sample, not safety, and a plan that wants gentle trims writes small steps. Piezo inside a measurement plan is still refused.

22. **A guard that tests whether a thing EXISTS cannot tell it from a placeholder wearing its name — and on 2026-09-24, on the microscope computer, that defeated four of this repository's own guards at once, every one of them in the direction that looks fine.** On Windows `python3` is an App Execution Alias: a real file on PATH that `command -v` finds, which prints `Python was not found` and exits 49. Every interpreter guard here tested existence, so the `|| command -v python` fallback never fired in `.mcp.json`, in `.claude/mcp-preflight.sh`, or in `contracts/hooks/pre-commit`. Measured rather than inferred: the alias exits **49** under `-c ''` and the real interpreter exits 0, which is the whole of the fix — probe by running, not by finding.

    **What each did instead of working.** The librarian server never started, which is the `CONNECTION_CLOSED` a session opens with. `.claude/settings.json`'s PostToolUse validator reported a failure naming no cause. `contracts/hooks/pre-commit` refuses every commit with an **empty failure list**, because it greps the interpreter's complaint for `FAIL` and finds nothing — precisely the *"pre-commit: contracts, exit 1, silence"* shape recorded in its own comment from 2026-09-22. Same symptom, second cause, and the hook still cannot name either. And `.claude/mcp-preflight.sh` — whose comment says the machine where this check matters most is the one where naming `python3` would make **the checker** the thing that goes silent — was the checker, on exactly that machine, silent, because `|| exit 0` makes a check that could not run indistinguishable from a clean bill of health. It guarded against its own failure mode in prose and implemented it.

    **Two more of the same class, the same morning, in `contracts/validate.py`.** It reads text with no encoding, so under a cp1252 default it raises `UnicodeDecodeError` before checking anything — 204 such reads across 44 files, in all four agents and `contracts/`. And it builds `rel` with `str(p.relative_to(REPO))` while `ALLOWED_PATHS`, `SHARED_PATHS`, `DESIGN_OWNED` and `fixture_group` all split on `/`. Under `PYTHONUTF8=1` the run is `147 passed, 970 failed`, and **964 of the 970 are check 13 alone, all false.** The same backslash defeats `fixture_group`, so the 20 group fixtures are read as flat cards, `if groups:` never fires, and `--expect-fail` prints `81/81 cards rejected as intended` **with the group half never evaluated** — a suite that reports complete and is not. It also defeats `seat_boundary_of`, which is what check 41 uses to hold one seat out of another's paths, so on this platform the boundary gate misclassifies every path it is given.

    **And a fifth, one layer down, the same afternoon.** After the interpreter fix the server answered an MCP `initialize` handshake correctly through the launcher from a shell -- and a restarted session still opened with `CONNECTION_CLOSED`. `.mcp.json` names `sh` as its command, and `sh` is **not on the Windows PATH**: Git for Windows installs it under `Git/bin` and `Git/usr/bin` and puts only `Git/cmd` on PATH. Hooks run inside Git Bash and find it, which is why the preflight spoke in the same session whose server was dead; an MCP server is spawned directly as a Windows process and does not. And `bash` on that PATH is the WSL launcher, a second App Execution Alias. The file's own comment said *sh and git are both present wherever this repository was cloned, including Windows* -- present on disk, absent where the spawner looks: existence against resolvability, the same defect as the interpreter, in the very thing written to resolve the interpreter. The remedy is the machine's (`Git/bin` on the user PATH, then restart the app), because no one command name is right on every platform and a tracked file must not carry a machine path. **Confirmed the same afternoon**: the person added `Git/bin` to the user PATH and restarted the app, and the next session reported the librarian `connected` with four tools -- the first time the service answered a session on this machine. Checked by the app's connector status and not by a query, because the seat that checked has no question to ask the store.

    **What is open is not the fixes.** Those are specified and owned: the three configuration files are architecture's and landed 2026-09-24; the validator and the hooks are a manager's; the 204 call sites are each agent's. What has no home is that **every check here runs inside the validator, and the validator was the broken thing** — so nothing in this repository can report *"the machinery does not work on this machine."* §11-7 asks for fixtures that test the gate itself and §11-18 records permissions living where no check reaches; this is the third and widest of that family, because a second platform had never been run and a platform assumption stays invisible until someone stands on it. **The cheapest partial answer already exists and went unused**: `CLAUDE.md` says to read every count off a run and says `--expect-fail` prints *two* totals. Noticing that only one printed is what found the group hole. A check cannot check its own runner; a reader counting what a run actually prints can.

21. **An approved operation that produces no result card.** First focus-finding moves Z, so it is S6 and Tier 1-2 and needs approval, and **its output is an operating state rather than a measurement**. The bare-particle pre-measurement is the same shape -- a short acquisition to get `tracer_brightness` and `bleaching_rate`, a measurement taken in order to make a measurement possible. §4.4 says an envelope carries a `plan` or a `result`, and neither of these has a result to carry; §4.5's five stages may or may not have a slot for it. **Both are approved operations that come before the measurement and settle nothing about the world**, and until they have a home they will be written as whatever is nearest. Raised by `microscope-5` designing the first focus search.

    → **Settled 2026-09-24 for the second of the two, on the day it first happened.** Today's first real acquisition on the microscope computer had no plan that fit it -- the only plan was 100x oil with software motion and 100x axis ranges, and a 20x revision citing those would launder them -- and `run_log.schema.json` requires `plan_id`. `manager-microscope-20260924-1` asked and recommended the answer taken here. **A run may exist without a plan, as a preparatory run, on four conditions, every one checkable from the log:** (1) `plan_id` null with a required `no_plan_because`; (2) the command list the person approved before light, by path and sha256, since that approval is what stands where a plan's `scope_approval` would; (3) **nothing moves** -- no dispatch to any device in the motion set (defined in check 85: what changes where the objective, the sample or a trapped object sits; selectors are outside it), which is also why first focus-finding, the other half of this item, stays a planned operation: it moves Z, so it needs a plan; (4) **it produces no result card and no KB entry of its own.** Its numbers reach the world only when a later plan cites its `run_id` as an input. That is not a plan coming after the run it governs -- it governs nothing -- but a plan standing on an earlier measurement, which is ordinary. The alternative, keeping the log out of `runs/` until a plan exists and then filing it under that plan, was rejected because it makes the record say a plan preceded a run it did not. **The schema field, the check and fixtures watched failing are the manager's.** Until they land, a preparatory run's log lives beside its frames outside the tree with its sha256, and no invented `plan_id` is written anywhere. **An operation plan, for a move that verifies rather than measures** (settled 2026-09-24, the first half of this item). A verification move -- a small step on one piezo axis to check which channel moves which way, or the step that finds which Z direction approaches the objective -- measures nothing about the sample, so S3's seven axes have nothing to decide, and demanding them would push the move off the record rather than onto it. It is still motion, so a preparatory run cannot carry it. **An operation plan is a plan card with no S3 to S5**, on six conditions: (1) one device, one axis per action; (2) every commanded position checked against the envelope's limits by deterministic code before anything is sent, and refused outside them, a missing limit refusing too; (3) each move's size and count written in the plan and approved by the person, with the bench (§6.2.1); (4) position read back before and after each move -- and **a resting read-back may fall short of a floor by the person's read-back tolerance and still count as at it** (the person, 2026-09-24: 10 nm on every piezo axis, after Z was found resting at -1.7 nm and the wrapper rightly refused every Z command). It lives in `envelope/safety.json` beside the limits it qualifies, because it loosens a safety comparison and so is the person's number; it applies only to reading where an axis sits, never to a commanded position, which still may not cross the floor at all; and it is not the operator widening a limit at run time, which stays forbidden, because the person wrote it; (5) the plan states what it verifies and what observation would falsify it; (6) reversible moves only. **A bounded periodic motion qualifies** -- a sine of stated centre, amplitude, period and cycle count, host-timed, with every point checked before the first is sent -- on X and Y only, and not on a controller's own waveform generator while its unit is unmeasured. **Z gets only the direction-finding step** until that direction is recorded, and `objective_clearance_min` binds it. What such a plan finds -- a verified channel mapping, a measured direction, a tracking error -- is a fact about the instrument and leaves as a result card to the librarian, naming the observation that verified it. **What it does not cover**: any measurement bound for the store, which still takes S3 to S5; first focus-finding, which approaches the coverslip and waits for its own design (card 026); anything irreversible. **The tweezers qualify too, with two conditions substituted** (architecture, 2026-09-25, on `manager-microscope`'s reading at `5ab8116`, which is right). The piezo was the first case and not the definition: what makes an operation plan is a move that verifies and measures nothing, and placing a trap for the person to confirm a bead is that. So a plan carrying `trap_steps` and no goal takes this branch, and one with a goal stays a measurement plan with the full pipeline. On a blind channel two conditions cannot hold as written, and each is replaced rather than dropped. **Condition 4's read-back becomes the person's statement**: a `hold_for_person` follows every step whose outcome a later step relies on, such as a caught bead or a trap the person has deleted. A camera frame may record it, but a frame is not a read-back of the trap. **Condition 1's one axis per action becomes one trap per step**: a trap position is one absolute point, and both of its coordinates are checked. The rest hold as written. Limits are checked before anything is sent: the person's `tweezers_trap_*` for the objective the person states at hand-over, a missing one refusing, enforced at run time by the trap-step gate (`759e9f0`) and, as declared, at commit time too (below). Step size and count are written and approved. `verifies` and `falsified_by` are stated. Every step is reversible. Asked by `manager-microscope-20260924-1` when the person chose to check piezo X with one watched step before driving it with a sine. **Condition (4) bars measurements, not what a device says about itself** (narrowed 2026-09-24, the evening of the same day). It was written for numbers like `bleaching_rate`, which need a plan that declares the estimator before the data. It also caught a second kind the day produced in quantity, and should not have: **a device's report of itself** -- which camera body answered to a label (by serial), where a piezo axis rests, the command set a controller lists, a setting read back after it was written. Those are read-backs with nothing to estimate and no estimator to declare. So the librarian may enter such a report citing a preparatory run's log as its source, with the `run_id` and the event it came from, graded as an observation by this system. **The line is the vocabulary**: a value of a registered observable never enters this way and still waits for a plan that cites the run; a self-report does. The person asked that the day's findings be saved properly before the architecture session closed, and this is what that required. Check 85's half is the manager's.

    **A stand-in script is for a preparatory run only** (added the same day). The first two runs went through a session script because the plan dispatcher could not reach the hardware -- the registry carries no Micro-Manager labels, and `derive_commands` produces no settings to verify. **A planned run, whose result card goes to the librarian, goes through the dispatcher**, because what makes a measurement planned is that its commands are derived from the plan mechanically rather than written beside it by hand. So those two gaps are closed before the first planned run on this machine, and if they cannot be, the question comes back here rather than being routed around. Settled when the person chose that the photostability measurement bound for the store is the next repeat under the 20x plan, with the two runs before it cited as inputs.

20. **A model choosing among measured values is neither P0's case nor P2's.** P0 keeps a model out of safety decisions and P2 keeps a model's invented number out of every card; **selection is neither**. Jev picks one scan frame and the number comes from that frame's encoder, so the source stays hardware and the grade is the encoder's. **The grade is exactly what does not capture the risk**: pick the dust plane instead of the particle plane and the reading is perfectly accurate and perfectly wrong. So what a selection needs is **a falsifier, not a grade** -- something that can be checked against afterwards -- and this document has no shape for one. `microscope-5` is borrowing `configuration_preference`'s form meanwhile and says so, which is the right way to borrow.

19. **Two things `thr-tracer-diffusivity-001` must not be closed without, held here because the seat that would hold them is empty.** This is a pointer and deliberately not a copy: the content is on disk in three places already, and a fourth copy is the thing that goes stale while looking current. The librarian asked for exactly this and asked for nothing else.

    **(a) The quantity that thread compares now has a third name, and the third is not a synonym.** `contracts/quantities.json` carries it under `open_collisions`: `diffusivity` (simulation's local word, six uses), `tracer_diffusivity` (the registered observable, a measurement), `tracer_diffusivity_expected` (a derived entry, Stokes-Einstein, E4). The first two are one quantity under two words and fail as a match. The third **is a different quantity**, so it does not fail — it substitutes, and a comparison runs on the wrong operand and reports. The bridge's whole job is putting a measurement beside a model, so a prediction landing in either slot compares a prediction with a prediction. Nothing about it looks wrong: the number is plausible, the name differs by one word, the grade is E4 like its neighbours.

    **(b) §5.8.1 changes that thread's verdict.** With the diameter an E2 measurement, the microscope's 5 um against simulation's `assumed:` 2 um is 2.5x, inside P15's band and therefore formally a tie -- and a tie is a verdict that carries the worse grade of what it compared. So "no difference" here is an E5 statement, and saying it ungraded overstates it. **Check 63 is exactly this rule**, and check 63's owner is `manager-bridge`. The warning and its enforcement are waiting on the same empty seat, which is why neither could be delivered: with 59 and 65, three checks now sit unowned.

    **Why it is in §11 and not in `bridge/`.** `bridge/CLAUDE.md` and `bridge/tasks/` belong to `manager-bridge`, so neither the librarian nor the simulation manager could write there, and architecture will not write into an agent tree either. This is check 50's finding one level up: **a delivery path and a seat with a reason to open it are two facts, and the first alone is a dead letter.** Check 50 tests the path. Nothing tests the second, because the second is a person seating someone. Four attempts to deliver this failed before it was raised.

18. **Part of a session's permissions lives where no check and no seat can reach it — what, if anything, this repository does about that.** `.claude/settings.local.json` is untracked and gitignored globally (`**/.claude/settings.local.json`), and on 2026-09-19 this repository's copy held one key: `{"disabledMcpjsonServers": ["librarian"]}`. A deny beats an allow, so the user-level approval that had been in place since 20:00 that evening did nothing, and three seats ran a day on the degraded path. `CLAUDE.md` diagnosed it as split per-directory approval and prescribed exactly the user-level approval that was already in force — **a wrong cause and a prescription that had already been followed.** Corrected 2026-09-20; found by simulation-2, which was the seat without tools and therefore the only one positioned to look.

    **The general shape is §2.1 rule 9's**: a protection written down, and a door beside it nobody can look through. The commit gate's soundness rests on an assumption — that the tree a commit would create contains everything that governs the repository's behaviour. **For permissions that assumption is false.** The gate unpacks the index to a scratch directory, and this file is in no index. Check 53 exists to audit settings files and globs `settings.json` exactly, so it walks past a file sitting beside the one it reads; its own note about refusals it cannot see names user-level and harness ones, not this. Three seats recorded the symptom over a day and none could state the cause from inside its boundary, because the cause was not inside anyone's boundary.

    **Settled 2026-09-20: none of the three, and the person chose a fourth.** The three below were (a) a habit, (b) a reporting check over the working copy, (c) tracking the file, and the answer is **carry the approval instead of the refusal**: `.claude/settings.json` -- checked in, in the tree, read by the gate -- now holds `enabledMcpjsonServers: ["librarian"]`, so a fresh clone on any machine has the approval without anyone remembering a step. It does not fight the gitignore, it does not make a check depend on the machine, and it does not rely on a person. **Named rather than `enableAll`**, so a second server appearing in `.mcp.json` later cannot ride in on this decision. The cost is that cloning this repository means its own read-only server runs without being asked -- accepted, since a seat that clones it is going to run this code anyway.

**And the silence is answered separately, because the approval does not cover a deny.** A local `disabledMcpjsonServers` still beats it and still sits where no check can look, so `.claude/mcp-preflight.sh` runs at SessionStart and says so: it reads configuration only -- `.mcp.json`, the shared settings, `.claude/settings.local.json`, `~/.claude.json`'s entry for this directory, and `python3` on PATH -- and **executes no agent code**, because reading or running `librarian_agent/src/` is that agent's side and a root hook is not a seat that may cross it (§6.2 rule 3). So it cannot prove the tools are live, only that nothing is refusing them; the proof of life is still the seat calling one tool, which costs nothing. **It reports and never blocks** -- degraded is a legitimate state here. Both branches were run before this was written: healthy is silent, and a planted `{"disabledMcpjsonServers": ["librarian"]}` prints the file's path and what to remove.

**And candidate (b) is granted on 2026-09-22, which reverses the refusal above -- what changed is that it no longer has to carry the job alone.** When it was refused there was **no guard at all**, so a machine-dependent check would have been the whole answer and its weaknesses disqualified it. `.claude/mcp-preflight.sh` now warns the session that is starting, at the moment that session can act. So check 76 does not need to be the guard; it needs to make the absence **visible in the shared artifact**, which is the failure that actually happened -- three seats quoting symptoms to each other with none able to state the cause. **Different audiences and different moments**: the hook tells the seat arriving, the check tells anyone reading a run, including a seat diagnosing someone else's problem. That is why it is not the same mechanism at a second address, which is what this document has refused four times this week.

**What made the three below wrong is worth keeping.** Three candidates, none obviously right. **(a) Nothing but a habit** — the file is the harness's, not the repository's, and the fix is the sentence now in `CLAUDE.md`: when tools are missing, read it first, by hand. Cheapest, and relies on a person remembering. **(b) A reporting check over the working copy** — it can be read on a bare run and never in the gate, so it would be the first check whose verdict depends on the machine rather than the tree, which is a property this validator does not currently have anywhere and may not want. **And the file moves faster than a check would be read**: on 2026-09-20 it went from the deny key to `{}` to absent inside ten minutes, twice between one seat's two reads of it, so such a check would report a different answer minute to minute and each report would look as settled as any other. **(c) Track the file** — this fights the global ignore for a reason the ignore is right about: the file holds per-machine approvals, and committing one seat's would push it onto every other working copy. Architecture raises it; the choice between (a) and (b) is a manager's to make, and (c) needs the person.
...
17. ~~**Should manager seats get worktrees**~~ → **Settled 2026-09-19: no. One shared working copy.** The person decided, after seeing both sides.
    ...
    **And the two defences against them are asymmetric, which is new on 2026-09-21.** A pre-commit hook can refuse to *take* another seat's hunk, because that happens before the commit and the hunk is there to see -- `manager-microscope` added one at `d90ca2f` and it caught a real pair with no false positives. Being *overwritten* has no such moment: the edit is gone before any hook runs, and it can only be **counted afterwards**. Four seats touched one file five times today and one revert-and-redo went through it. So one direction gets a guard and the other gets a census, and a seat that knows the hook exists may believe both are covered.
...
16. **There are two places an interval lives in an axis card — what compares them.** `inequalities[].interval` and `constraints[]` (the latter simply an array of `$ref: interval`), the same shape in two places. It may be a new row for §11-11 with an empty third column, and it may not — because the two may mean **different things** (one a per-inequality verdict, one the axis's output). The microscope manager raised it on 2026-09-19. — **Counted and closed on 2026-09-20: the answer is never, and it was never a §11-11 row.** Across 13 axis cards `inequalities[]` holds 37 entries and **`inequalities[].interval` is used zero times**; the 3 `returned` entries carry `allowed_set` and `precondition` instead, which is §5.3.2 working. `constraints[]` holds 6 parameters, all in the simulation axis cards -- **6 of them, not the 4 first reported**; `simulation-4` counted its own tree and corrected it, which is the report route working exactly where this entry says architecture cannot check. **Cards where both cover the same parameter: 0.** §11-11 is about one fact in two places and there has never been a second copy, so the unused slot is the intended home going forward and the question is convergence, not divergence -- nothing to check. **Counted by `manager-bridge`, and architecture cannot check it**: the counts live in both agents' `questions/`, the bridge holds §6.2 rule 3's exception to read there and this seat does not, and no check reports these totals in a run. So it closes on a report -- the designed route, written down here rather than passed off as verified.
...
4. **The run says whether the watcher was on, and `degraded` is not where that is said (sharpened 2026-09-22).** This item first said *the same mechanism as `degraded` applies*, and `microscope-5` showed why it cannot: `degraded`'s elements are backed by an append-only log that check 45 tests, and an external model has no such log, so mixing an unbacked element in **weakens what an empty `degraded` means** -- from *the librarian answered, and that was checked* to *the librarian answered and nobody knows about the other one*. **The record is the run log**, where item 2 already puts the watcher's output: events present means it spoke, absent means it did not, and nothing else has to carry it. No field is added and `degraded` keeps one kind of element.  A dead API is silence, and silence looks like a good run; the same mechanism as `degraded` (§3.1) applies, so no run reads as watched when it was not.

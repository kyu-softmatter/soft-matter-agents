...
|---|---|---|
| 76 | whether a `settings.local.json` sitting beside a tracked `settings.json` is reported -- its existence and the NAMES of its MCP and permission keys, never their values. Advisory by construction and says so: the file is in no tree, so the gate can never see it and only a working-copy run can | manager-microscope |

...
> **서로 다른 축은 상대의 변수를 몰라도 각자 구간을 낼 수 있어야 한다.**

구동과 운동은 이탈 조건으로 직접 묶여 있다. 갈라놓으면 "운동 축"이 구간을 내기 위해 트랩 강성을 알아야 하는데 그 값은 "구동 축"의 출력이다 — **형제를 봐야 하는 축**이 생기는 순간 S3의 독립성(§4.5.2)이 무너진다. **한 부등식 안에 같이 들어가는 변수들은 한 축에 있어야 한다.** 새 축을 만들 때 이 기준을 먼저 통과시킨다.
...
| 피에조 스테이지 | 미세 위치, 벽으로부터의 거리 | 이동 범위·속도·대역폭·정착시간, 폐루프 여부 |
| 모터라이즈드 스테이지 | 거친 위치, 시야 이동 | 속도·가속 한계, 백래시, 반복도 |
| 옵티컬 트위저 | 포획, 트랩 강성, 힘 인가 | 출력, **트랩 개수 상한과 분할 방식**, 트랩 위치 제어와 이동 속도 한계, 강성 캘리브레이션 경로 |
...
현재 현미경에 붙은 아홉 종류의 장치 — 본체, 카메라 2대, 형광 광원 2개, DMD, 컨포컬 레이저, 컨포컬, 피에조 스테이지, 모터라이즈드 스테이지, 옵티컬 트위저 — 는 **각각 다른 소프트웨어나 입출력 단자로 조작된다.** 이 사실이 operator 설계에 강제하는 것:
...
rebuild/
  ...
  microscope_agent/                 최상위 폴더 3개 + CLAUDE.md
    ...
    src/                            결정론적 코드 (§7.2)
                                      ...
                                      devices/          dev_*.py ×11, manual.py, mock.py

...
| **M4** | **One bridge round trip.** Hashes, unit and dimensionless consistency, producibility | Producibility derives only once both sides' `capabilities/` are filled **and** each side has at least one card -- the §11-1 half of this closed on 2026-09-19 and the `capabilities/` half did not | One side's result → the other side's plan → the results compared as the same observable (S4) |

...
#### 4.3.0 저장소가 먼저, 서비스는 나중 (§9)

**사서는 두 가지다: 지식 저장소와 그 위의 서비스.** 구현 순서상 저장소가 먼저 존재하고(M1 시작 시점), 에이전트화는 M3다.

| | 저장소 (M1부터) | 서비스 (M3부터) |
|---|---|---|
| 무엇 | `kb/entries/`, `kb/sources/`, `kb/distilled/`, `kb/index.json` | MCP 서버, 컨텍스트 질의, `gaps` 산출, 증류, 외부 검색, 충돌 탐지, 스냅샷 내보내기 |
| 누가 쓰나 | **사람이 손으로 큐레이션한다** (`curated_by`) | 사서 에이전트 |
| 누가 읽나 | 에이전트가 파일을 직접 읽고 `kb:` 출처로 인용 | `kb_query` 호출 |
...
**서비스가 없을 때 실제로 잃는 것**은 값이 아니라 **빈칸의 발견**이다: 무엇이 KB에 없는지(`gaps`), 어떤 주장이 서로 어긋나는지(`conflict`), 외부에 답이 있는지를 알 수 없다. 그래서 M1–M2의 카드는 `degraded: [librarian_agent]`를 달되, 그 뜻은 "문헌값이 없다"가 아니라 **"무엇을 놓쳤는지 모른다"** 다.

#### 4.3.1 인터페이스 — MCP 서버 (D9, M3부터)
...
**E5는 허용하되 센다.** 계획서 하나에 E5가 몇 개인지가 그 계획의 신뢰도이며 상한이 있다(§11-3). E6은 어디에도 들어갈 수 없다.
...
rebuild/
  ...
  librarian_agent/                  시스템의 유일한 지식 저장소 (P14)
    ...
    src/kb_index.py                 인덱스 재생성 (저장소 유지관리, M1부터)
    src/mcp_server.py               읽기 전용·caller 격리 MCP 서버 (§4.3.1, M3)
    src/export_snapshot.py          각 에이전트의 envelope/snapshot.* 생성 (M3)
...
구현 순서는 **현미경 → 시뮬레이션 → 사서 → 브리지**다.

| | 내용 | 완료 조건 |
|---|---|---|
| **M0** ✔ | **계약 먼저.** 카드 스키마 + `units.json`/`units.md` + `validate.py`(검사 37개) + `capabilities/` 골격 | **완료.** 사람이 손으로 쓴 goal → axis → synthesis → plan → 승인 → result 왕복이 검증기를 통과하고(`0 failed`), `rejected/`의 카드 8장이 의도대로 전부 거절된다(`--expect-fail`). 에이전트 코드는 없다. |
| **M1** | **현미경 에이전트.** S2 정교화 → S3.0 스크리닝 → 축별 해석 → S4 종합 → S5 산출 → 승인 → 실행. `envelope/safety.json` 기계 판독화(**사람이 쓴다**, §10.3 규칙 4), 광경로 표와 `capabilities/`의 연결(검사 38), 승인 게이트 hook | 승인 없이 Tier 2 호출이 **실제로 차단**된다. mock 백엔드로 파이프라인 1회, 그다음 실측 1건이 편차 기록과 함께 남는다. |
| **M2** | **시뮬레이션 에이전트.** 같은 5단 파이프라인, 축 A1–A5, HOOMD 백엔드 | 목표 1건 → 검증 통과한 plan → 스모크런 → 수렴 증거 |
| **M3** | **사서 에이전트.** entries 스키마, 증류, MCP 서버(읽기·`caller_id` 격리), `gaps` 반환, 스냅샷 내보내기 | 실제 질의 1건에 entries + gaps가 돌아온다. E6이 KB에 없다. 현미경·시뮬레이션의 `degraded: [librarian_agent]`가 사라진다. |
| **M4** | **브리지 왕복 1라운드.** 해시, 단위·무차원수 일관성, 대응 가능성 | 한쪽 result → 반대쪽 plan → 결과가 같은 관측량으로 비교된다(S4). |
| **M5** | **회고 루프.** `failures.jsonl` → 사서 증류 → `kb/lessons/`, 편향 지표 4종(§8.2) | 같은 질문의 두 번째 왕복이 더 빠르고 모순이 없다(S5). lesson이 후보를 지우지 않음이 검사로 확인된다(검사 31). |

**현미경이 먼저라는 것은 사서를 에이전트화하는 일이 뒤로 간다는 뜻이고, 지식 저장소가 없다는 뜻이 아니다**(§4.3.0). `kb/`는 M1 시작 시점부터 **사람이 큐레이션하는 저장소**로 존재하고, 에이전트가 파일을 직접 읽어 `kb:` 출처로 인용하며, 검증기가 그 등급을 저장소와 대조한다(검사 21·25).

M1–M2 동안 카드는 `degraded: [librarian_agent]`를 달지만, 그 뜻은 **"문헌값이 없다"가 아니라 "무엇을 놓쳤는지 모른다"** 다 — `gaps` 산출, 충돌 탐지, 외부 검색이 없기 때문이다.

따라오는 결과 셋:
1. **E5 상한(§11-3)은 M3 이후에 정한다.** 빈칸을 자동으로 발견하지 못하는 동안의 E5 개수는 "아무도 찾아보지 않은 값"을 포함하므로 정상 운영의 표본이 아니다. 그때까지 검사 3은 `UNDECIDED`로 남는다 — 아무도 고르지 않은 임계값은 만족된 임계값이 아니다.
2. **`gaps`는 M3부터 자동으로 채워진다.** 그 순간 M1–M2에 쌓인 E5 중 무엇이 실은 문헌에 있었는지가 드러난다. 그 차이가 서비스의 값이다.
...
2. **E5 상한**: 계획서 하나에 추정값을 몇 개까지 허용할까? 0이면 아무것도 못 하고, 무제한이면 등급 체계가 무의미하다. **M3 이후에 정한다**(§9) — 그때까지 검사 3은 `UNDECIDED`다.
...
5. **사서 MCP 도구 목록의 확정**: `kb_query / kb_get / kb_conflicts` 셋으로 충분한가? 조건 범위 질의의 인자 모양은 어떻게 잡을까(§4.3.1). M3 과제다.
...
**저장 형태는 최종이 아니다.** 두 파일은 평평한 표이고, "한 entry = 한 원자적 주장"(§4.3)을 지키지 않는다. 의도된 것이다 — 올바른 분해는 **어떻게 질의될지**에 달려 있고, 질의하는 쪽(사서의 `kb_query`, 조건 범위 매칭)이 M3까지 없다. 지금 마흔 개 entry로 쪼개면 추측 위에 형태를 고정하게 된다.
...
KB가 아직 없으므로(pre-M0) 사람이 준 사실을 임시로 모아 둔다. **M1에서 각 항목은 출처와 등급을 갖춘 KB entry가 되고, 이 절은 비워진다.** 여기 있는 동안 이 값들은 계획의 근거가 될 수 없다(P2, P14).

...
| D12 | 지시·보고 계층 | **3계층.** 아키텍처(구조를 쓴다) → 매니저(각 에이전트의 설계 수정, worktree 머지 검토) → 에이전트 넷(수행). 지시는 내려가고 보고는 올라온다. **서브세션은 각자 `git worktree`를 갖고**, 통합은 매니저의 머지라는 한 지점을 지난다. 계층은 권한을 늘리지 않는다 — 위의 둘은 Tier 0이다(§6.2, §6.2.1). |
...
| 계층 | 세션 | 권한 상한 | 쓸 수 있는 곳 |
|---|---|---|---|
| 아키텍처 | 아키텍처 | Tier 0 | `plan.md`, `contracts/seats.json` |
| 매니저 | 매니저 | Tier 0 | `contracts/`(`seats.json` 제외), 각 에이전트의 `CLAUDE.md`·`.claude/` |
| 에이전트 | 현미경 | Tier 2 (승인 있을 때) | `microscope_agent/` |
| 에이전트 | 시뮬레이션 | Tier 2 (승인 있을 때) | `simulation_agent/` |
| 에이전트 | 사서 | Tier 1 (외부 검색, KB 쓰기) | `librarian_agent/` |
| 에이전트 | 브리지 | **Tier 0만** | `bridge/` |

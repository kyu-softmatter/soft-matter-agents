...
└───────────────────────┬────────────────────────────────────────
                        │  고정 인터페이스 (§4.6.5)
        ┌───────────────┴────────────────┐
   backends/hoomd.py                backends/<device>.py
   backends/mock.py                 backends/mock.py
      (HOOMD-blue)                  (광원·스테이지·카메라·온도)
```
...
| | 현미경 모듈 | 시뮬레이션 모듈 |
|---|---|---|
| MOD1 | 광원·트랩 (출력, 강성) | 초기 구성 생성 (배치, 밀도) |
| MOD2 | 스테이지·위치 (벽 거리, 이동) | 적분기 설정 (시간간격, 열욕) |
| MOD3 | 검출·카메라 (프레임레이트, 노출, ROI) | 관측량 측정 (저장 간격, 계산) |
| MOD4 | 환경 (온도, 대기 시간) | 체크포인트·저장 |
| MOD5 | 수집·저장 | 잡 제출·자원 |

모듈은 **자기 파라미터만** 받는다. 모듈 간 직접 통신은 없고, 순서와 인터록은 operator가 쥔다 — S3의 병렬 독립성과 같은 이유다.
...
rebuild/
  ...
  microscope_agent/                 최상위 폴더 3개 + CLAUDE.md
    ...
    backends/                       <device>.py, mock.py  (고정 인터페이스, §4.6.5)
  ...
  simulation_agent/                 위와 같은 구조, 축 목록만 다름 (§4.5.3)
    ...
    backends/                       hoomd.py, mock.py     (고정 인터페이스, §4.6.5)
...
8. **모듈 분해와 backend 인터페이스**: §4.6.3의 5개 모듈과 `preflight/apply/read/abort` 네 함수로 실제 HOOMD-blue 호출과 장비 제어를 덮을 수 있는가? 부족하면 무엇이 빠지는가.
9. **축소 모드의 전파 범위**: `degraded` 표시를 시스템 내부에서만 쓸까, 외부 보고(그림·논문)까지 끌고 갈까?
10. **세션 경계**: 4개 에이전트를 4개 Claude Code 세션으로 항상 분리할까, 한 세션에서 디렉터리 스코프로 전환할까? (권한 격리는 전자가 강하고, 왕복 속도는 후자가 빠르다)

...
**기다리는 동안 노는 것이 아니다.** 참고 자료를 읽고 §10.2.1로 판정해 `tasks/NNN-rulings.md`에 적는 일은 지금 해도 되고, 그것은 비교 대상 카드를 만들지 않는다. 다만 **준비가 한쪽으로 기울면 출발선이 기울므로**, 출발 전에 한 판정은 전부 그 파일에 남아 있어야 한다 — 비대칭이 보이지 않으면 비교가 그 비대칭을 재게 된다.

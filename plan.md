...
**비울지를 정하는 것은 한 단 위다**(2026-09-18). 실행석의 초기화는 그 에이전트의 매니저가, 매니저의 초기화는 아키텍처가 정한다. 아키텍처의 것은 사람이 정한다 — 위가 없으므로 그 한 칸은 비워 둘 수 없고, 비워 두면 가장 오래 사는 컨텍스트가 아무 규율도 받지 않는다.

**자기가 정하지 않는 이유**는 판단 근거가 자기에게 없기 때문이다. 컨텍스트가 길어진 세션은 자기가 무엇을 컨텍스트에서만 알고 있는지 볼 수 없다 — 그 지식이 디스크에서 온 것처럼 느껴진다. 위에서는 보인다.

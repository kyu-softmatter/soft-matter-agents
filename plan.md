...
**The fields landed on 2026-09-20 and check 54's code did not, so the first synthesis card will turn the tree red on purpose.** `8bcee35` added `allowed_sets`, `preconditions` and `unbounded` to `synthesis.schema.json` and lifted `bound_state` and `unbound_kind` into `common.schema.json` so the two contracts cannot drift on what `abstained` means. Check 54 still reads `refs = {r['entry_id'] for r in card['kb_refs']}` -- **the card's own** -- so a carried `kb:` basis will fail against a synthesis card that correctly does not cite what it never asked for. **That red is the ruling arriving before its implementation, not a defect in the card**, and it is written here so nobody spends an afternoon on it. The seat that added the fields declined to also change 54, on the ground that it belongs to a vacant seat and is a separate decision -- the right instinct, and the same one it had broken and recorded earlier the same day (§6.2.2).

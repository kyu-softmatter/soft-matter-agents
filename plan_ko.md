...
**`inputs`도 `kb:<entry_id>`를 받는다 (2026-09-20) — `basis`와 같은 부류의 둘째 사례다.** 유도량의 `inputs`가 지금 카드 자신의 이름만 받으므로, **입력이 저장소에서 온 유도값은 표현할 수 없다.** 2026-09-20에 그것이 실물을 막았다 — 실행석이 `tracer_diffusivity_expected`를 계산해 놓고(τ_d ≈ 300 s, 시뮬레이션 쪽 독립 계산 312 s와 한 자릿수 안에서 교차) **적을 자리가 없어 되돌렸다.**
    ...
    **비대칭의 방향이 거꾸로라는 것이 이 결함의 이름이다.** 같은 세 입력이 **entry에서는 합법이고 카드에서는 불법**이다 — entry 스키마는 저장소에 대해 해소되는 이름을 받고, 검사 17은 카드의 `inputs`를 그 카드 자신의 `numbers[]`와 `CONSTANTS`로만 받는다. 그런데 **게이트에 근거를 보여야 하는 쪽은 카드이고, 근거를 댈 수 없는 쪽도 카드다.** 현미경 매니저가 그렇게 적었고 그 문장이 왜 이것이 결함인지를 가장 짧게 말한다.

    **그리고 고리가 닫혀 있었다 — 표현 간극이 미용 문제가 아니라는 증거다.** 팬아웃이 현재 저장소로 옮겨 가지 못하는 이유가 **옮기면 A2가 표현 불가능해지기** 때문이었다: `missing`이 비어 축이 `failed`로 떨어지는데 **그것은 멀쩡히 돈 축에 대해 거짓**이고 계획을 멈추는 상태다. 그래서 **이 한 필드가 재핀 일곱 장 전체를 잡고 있었다.** 그동안 실행석은 `tracer_diffusivity_expected`를 계산해 놓고(0.09 µm²/s, τ_d ≈ 300 s — 시뮬레이션 쪽 독립 추정 312 s와 한 자릿수 안에서 교차) **적을 자리가 없어 되돌렸다.** 두 에이전트가 독립적으로 같은 양에 닿은 첫 사례가 **메시지에만 살고 카드에 없었다**(§6.2 규칙 2).
